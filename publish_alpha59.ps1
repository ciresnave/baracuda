# Phase 42-59 (alpha.59) publish script.
# alpha.59 ships Phase 59a (FA2 FW head_dim fanout to all 6 v2.8.3 dims +
# GQA/ALiBi/sliding/softcap API) + Phase 59b (FA2 BW + varlen) + the
# Phase 59c flash SMEM-carveout race fix surfaced during Consolidation D
# regression validation. Crate roster unchanged from alpha.58.
#
...
$logFile = "target/publish_alpha59.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.59 publish run started $(Get-Date)" -NoNewline:$false
