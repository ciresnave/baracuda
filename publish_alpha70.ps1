# alpha.70 publish script.
#
# Device-load telemetry aliases for Fuel's DeviceLoadSelector (Step E, Phase B2):
# Stream::is_idle() (baracuda-driver + baracuda-runtime) and
# nvml::Device::gpu_utilization_percent() (baracuda-nvml). Both are thin read-only
# aliases over reads that already shipped in alpha.69. See CHANGELOG.md
# (0.0.1-alpha.70). No new crates; same topo order as alpha.69.
#
...
$logFile = "target/publish_alpha70.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.70 publish run started $(Get-Date)`n"
