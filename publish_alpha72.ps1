# alpha.72 publish script.
#
# Hardening of the alpha.71 stream-ordered free (post-ship adversarial review):
# DeviceBuffer::drop no longer falls back to a SYNCHRONOUS free when the async
# free CALL errors (only when the async-free SYMBOL is missing = pre-11.2); on an
# async-free call error it leaks rather than risk a sync free of in-use pool
# memory. Plus the single-origin-stream precondition is now documented. No API
# change, no new crates. See CHANGELOG.md (0.0.1-alpha.72) and
# docs/fuel-reply-stream-ordered-free-2026-06-29.md. Same topo order as alpha.71.
#
...
$logFile = "target/publish_alpha72.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.72 publish run started $(Get-Date)`n"
