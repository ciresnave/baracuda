# alpha.76 publish script.
#
# The live-JIT unblock: baracuda-kernelgen is PUBLISHED for the first time (the
# seam feature is the supported surface - Fuel constructs BaracudaSynthesizer
# from crates.io behind its jit feature), and FKC bundles now import through
# Fuel's real importer end-to-end (op_kind + bundle-schema reconciliations,
# adversarially reviewed; contracts can no longer lie - broadcast/flipped/fused
# cells withheld honestly). Plus the bespoke NaN-propagating relu family for
# Fuel's ReluElementwise rebind, the CI feature-coverage steps, the workspace
# clippy sweep, and the strict-2024 fmt pin. See CHANGELOG.md (0.0.1-alpha.76).
# baracuda-kernels-bench stays publish=false; the topo order gains
# baracuda-kernelgen (69 crates; deps: kernels-types + optional nvrtc + the
# published fuel-kernel-seam 0.10.3 crates).
#
...
$logFile = "target/publish_alpha76.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.76 publish run started $(Get-Date)`n"
