# alpha.73 publish script.
#
# The kernel-specialization release: baracuda-kernelgen grows into a value-
# numbered-DAG IR + backend that generates kernels across the bespoke surface
# (reductions, contractions, layout views, gather/scatter), the IR-expansion
# ramp 0a-#5 (each adversarially reviewed), the forge nvcc/cl.exe build-error
# fix, and the Model-A mixed-dtype dispatch key (ElementKind::U32, per-operand
# accept dtypes). See CHANGELOG.md (0.0.1-alpha.73) and the docs/fuel-* channel.
# baracuda-kernelgen + baracuda-kernels-bench are publish=false (not on the
# index), so the topo order is unchanged from alpha.71/.72 (68 crates).
#
...
$logFile = "target/publish_alpha73.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.73 publish run started $(Get-Date)`n"
