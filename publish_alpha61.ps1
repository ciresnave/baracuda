# Phase 61 (alpha.61) publish script.
# alpha.61 ships the Fuel in-place op family completion: 2 new bf16/f16
# affine_inplace FFI symbols + documented same-pointer aliasing safety
# on unary/binary/ternary contig elementwise trailblazers.
# Crate roster unchanged from alpha.61.
#
...
Set-Content -Path $logFile -Value "alpha.61 publish run started $(Get-Date)" -NoNewline:$false
