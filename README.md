...
| CUTLASS          |   ✅    | Plan-based `GemmPlan` + `BatchedGemmPlan` + `GroupedGemmPlan` (MoE-friendly variable-M-per-group). Float family (`f16`/`bf16`/`f32`/`F32Strict` SIMT/`f64` DGEMM) × `{Rcr, Rrr}` × `{Identity, Bias, BiasRelu, BiasGelu, BiasSilu}` on `sm_80`. Int8 (`S8`/`U8`) × `Rcr` × the same epilogue family × `{f32, i32}` bias. Caller-supplied workspace, capture-safe via pinned metadata, host-side `can_implement` for CUTLASS-vs-cuBLAS prelaunch branch. Hopper (`sm_90a`) deferred — see `crates/baracuda-cutlass`. Int8 RRR ships via the sibling `crates/baracuda-kernels` facade (bespoke `mma.sync` kernels — same plan API).                                                                                                                                                                                                                            |
