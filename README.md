...
![Tests](https://img.shields.io/badge/regression-2336%2F0-success)
...
**In active development — alpha.57.** **2339 GPU tests passing**
on an RTX 4070 (sm_89), across **619 binary targets** (Phase 56
adds the Ring Attention smoke binary + 3 new GPU tests). Phase 42-44
...
| 46 | FlashInfer cherry-pick — paged-KV decode + sort-free sampling + cascade attention (alpha.57, Phase 46 Checkpoint A): surgical extraction of three FlashInfer kernel families (Apache-2.0, v0.6.12, commit `eee0d75f`) vendored under `crates/baracuda-kernels-sys/vendor/flashinfer/` (~12 kLOC across 25 headers, no wholesale wrap). NEW plan families: `BatchPagedDecodePlan` + `PagedKvAppendPlan` (vLLM-style paged KV cache decode), `TopKTopPSamplingPlan` (sort-free TopK/TopP/MinP/combined samplers, single-kernel rejection-based — much faster than baracuda's existing argsort+softmax+multinomial), `CascadeAttentionPlan` (LSE-merge for prefix-cache sharing across requests — system prompts, RAG context). NEW `BackendKind::FlashInfer` + `RandomKind::Multinomial` discriminants. NEW `flashinfer` cargo feature on both `baracuda-kernels-sys` and `baracuda-kernels` (default OFF). 6 MSVC-portability patches to vendored headers (`__attribute__((always_inline))` → `__forceinline__`, `ushort` → `unsigned short`, `__attribute__((shared))` → `__shared__`, `cuda::fast_mod_div` → pure-C++ magic-number divmod, `DISPATCH_HEAD_DIM` drop the unused 512 arm, `sampling.cuh` unused `#include "topk.cuh"` removed). Existing `FlashSdpaPlan`/`SdpaPlan`/`WriteSlicePlan`/`KvCacheAppendPlan` (contiguous KV) stay unchanged — Phase 46 plans are NEW siblings. Checkpoint A ships sampling + cascade + paged-KV append launchers (~25 new FFI symbols); paged decode launcher hits an MSVC nvcc `cudaLaunchKernel_ptsz` template-deduction issue and ships as a Phase 46 Checkpoint B follow-up (Rust plan + FFI declarations are in place — only the .cu launcher is staged out). | partial (Checkpoint A) |
