# Phase 63 (alpha.63) publish script.
# alpha.63 ships FlashAttention saved-tensor wiring for downstream
# autograd integration: 1 new FFI symbol
# (`baracuda_kernels_fa2_sdpa_lse_size` dense sibling of the existing
# `_varlen_lse_size`) + load-bearing "LSE saved-tensor contract" docs
# on FW + BW trailblazers + new docs guide at
# docs/guides/fa2-saved-tensor-contract.md + 12 new tests (3 host
# helper sanity + 5 FW->BW roundtrip + 4 BW feature-surface
# backfilling fa2_backward_smoke.rs gaps).
# Crate roster unchanged from alpha.63.
#
...
Set-Content -Path $logFile -Value "alpha.63 publish run started $(Get-Date)" -NoNewline:$false
