# alpha.69 publish script.
#
# Kernel-specialization / Profile v1 kernel-seam release: new baracuda-seam crate
# (SeamHello handshake + baracuda_seam_hello), structure_key + structure_key_token
# in baracuda-kernels-types, and the baracuda-kernelgen generator (publish=false,
# not uploaded). See CHANGELOG.md (0.0.1-alpha.69).
#
...
# baracuda-seam is new in alpha.69 and has no internal deps — placed early.
$order = @(
...
$logFile = "target/publish_alpha69.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.69 publish run started $(Get-Date)`n"
