...
def emit_markdown(cells: dict[tuple[str, str, str], dict[str, float]]) -> str:
    ...
    out.append("")
    out.append("⚠️ **READ THE ±5% BAND WITH THIS.** These figures come from a machine")
    out.append("running ~17 other agent processes at 77–100% CPU; an unloaded baseline")
    out.append("is not obtainable on it, and the GPU sits at 1605 of 3105 MHz SM clock")
    out.append("even at 0% utilization and 57 °C (`SW Thermal Slowdown: Active`).")
    out.append("")
    out.append("**Measured, not estimated.** Of the cells that have been run more than")
    out.append("once and carry both sides, **12 of 12 vary by more than ±5% between")
    out.append("runs of identical code** — median ratio spread **2.57×**, worst")
    out.append("**5.34×**. ⚠️ **Three of the twelve FLIP DIRECTION**: e.g.")
    out.append("`mmvq_multim / M1_N11008_C4096 / f32` ranged from *baracuda 3.18×")
    out.append("faster* to *baracuda 1.68× slower* across ten runs, so its headline")
    out.append("verdict is not reproducible.")
    out.append("")
    out.append("⚠️ **And only 12 of ~457 cells have been run more than once.** The")
    out.append("rest carry a single observation, so their reproducibility is")
    out.append("**unmeasured** — not good, not bad, unmeasured. A `≈` or a bolded")
    out.append("`> 1` on a single-run cell states a comparison, not a repeatable one.")
    out.append("")
