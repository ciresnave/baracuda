# Phase 60 (alpha.60) retry script — handles dev-dep cycles by stripping
# `baracuda-*` lines from `[dev-dependencies]` sections.
# Based on publish_alpha60_retry.ps1 pattern.
#
# Note: baracuda-kernels-sys is FIRST because Phase 40 added a normal dep
# on baracuda-cutlass-kernels-sys, so kernels-sys was out of order in the
# main run — re-publish now that cutlass-kernels-sys is live.
...
$logFile = "target/publish_alpha60_retry.log"
Add-Content -Path $logFile -Value "`n=== alpha.60 publish retry started $(Get-Date) ==="
