# Phase 60 (alpha.60) publish script.
# alpha.60 corrects alpha.59's incorrect "permanently out-of-scope" claim
# for head_dims {160, 224, 512}: vendors 12 new FA2 FW .cu files from the
# Candle fork (EricLBuehler/candle for hd160/224 via Laurent Mazare +
# Michael Feil PRs; huggingface/candle for hd512 via Eric Buehler PR #3417).
# FW now covers all 9 head_dims {32, 64, 96, 128, 160, 192, 224, 256, 512};
# BW remains 6 head_dims (hd160/224/512 BW genuinely blocked by FA2 BW
# kernel atom_layout constraints — kBlockKSmem=32 for hd160/224 and
# kBlockM>=64 for hd512; matches upstream + Candle).
# Crate roster unchanged from alpha.59.
#
...
Set-Content -Path $logFile -Value "alpha.60 publish run started $(Get-Date)" -NoNewline:$false
