...
`rust-toolchain.toml` = `channel = "stable"` — a **moving alias, not a pin.** This box resolves it to **1.97.1** (2026-07-14); CI's `@stable` resolves **1.98.0** (2026-08-18). So local `cargo clippy`/`build` checks a **different diagnostic set** than the gate: a lint that postdates 1.97.1 reds CI on a locally-green tree. Exposure is **lint/warning reproducibility only, not numerics** — exact ops are strict IEEE-754 bit-identical across rustc versions, transcendentals go through the versioned `libm` crate, so a compiler bump does not change what the kernels compute. **Recorded, not fixed** — a portfolio-wide pin decision is with the maintainer; fixing it here alone is the one-decision-three-ways trap this audit exists to prevent.
