# alpha.77 publish script.
#
# Post-ramp codegen breadth + the CapturedRun _doff unblock: ten adversarially-
# reviewed increments on top of alpha.76 — the _doff WriteSlice (form B, a
# device-resident dynamic range-start so a captured KV decode write survives
# CUDA-graph replay; unblocks Fuel's CapturedRun) + nine baracuda-kernelgen
# increments (base_offset/rope, where-select/triu, hetero-multi/dropout, fused
# argsort, top-k/bottom-k, im2col, f64 scalar params, block-scan variants,
# partial-select top-k). Each design-scouted -> implemented -> adversarial
# multi-lens review -> on-device validated (RTX 4070, sm_89). kernels-types
# UNTOUCHED (no STRUCTURE_KEY_VERSION bump — additive keying). See CHANGELOG.md
# (0.0.1-alpha.77). Same topo order + crate set as alpha.76 (kernels-bench stays
# publish=false; deps: kernels-types + optional nvrtc + fuel-kernel-seam 0.10.3).
#
...
$order = @(
    ...
    "baracuda-kernels-types", "baracuda-kernelgen", "baracuda-nccl", "baracuda-npp", "baracuda-nvimagecodec",
...
$logFile = "target/publish_alpha77.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.77 publish run started $(Get-Date)`n"
