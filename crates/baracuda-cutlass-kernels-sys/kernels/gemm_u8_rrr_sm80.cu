// uint8 (unsigned) GEMM, RRR layout (RowMajor A × RowMajor B), sm_80
// instantiation.
//
// Computes: D = saturating_cast<uint8>(alpha * (A * B) + beta * C)
// with uint8 inputs, int32 accumulator, float alpha/beta. Output
// clamps to `[0, 255]` via `cvt.rni.sat.u8.f32`.
//
// Layout RRR:
//   A: row-major    [M, K]  (uint8)
//   B: row-major    [K, N]  (uint8)
//   C: row-major    [M, N]  (uint8, optional)
//   D: row-major    [M, N]  (uint8)
//
// See gemm_s8_rrr_sm80.cu for the tile-shape, epilogue rationale, and
// the vendored Crosswise-B `DefaultMmaCore` dependency.
...
using GemmU8RrrSm80 = cutlass::gemm::device::Gemm<
...
int baracuda_cutlass_gemm_u8_rrr_sm80_run(
    int m, int n, int k,
    const void* a, int64_t lda,
    const void* b, int64_t ldb,
    const void* c, int64_t ldc,
    ...
    void* stream)
{
    return baracuda_cutlass::u8_rrr::run_impl(
        m, n, k, a, lda, b, ldb, c, ldc, d, ldd,
        alpha, beta, workspace, workspace_bytes,
        static_cast<cudaStream_t>(stream));
}

std::size_t baracuda_cutlass_gemm_u8_rrr_sm80_workspace_size(int m, int n, int k) {
    return baracuda_cutlass::u8_rrr::workspace_impl(m, n, k);
}

int baracuda_cutlass_gemm_u8_rrr_sm80_can_implement(
    int m, int n, int k,
    const void* a, int64_t lda,
    const void* b, int64_t ldb,
    const void* c, int64_t ldc,
    void*       d, int64_t ldd)
{
    return baracuda_cutlass::u8_rrr::can_implement_impl(
        m, n, k, a, lda, b, ldb, c, ldc, d, ldd);
}
