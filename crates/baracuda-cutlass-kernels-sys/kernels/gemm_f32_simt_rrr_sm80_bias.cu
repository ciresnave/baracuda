...
//
// Mirror of gemm_f32_simt_rcr_sm80_bias.cu with B in row-major rather
// than column-major. Strict IEEE 754 binary32 throughout (inputs, accum,
...
template <typename ActivationOp>
using GemmF32SimtBiasRrrSm80 = cutlass::gemm::device::GemmUniversalWithBroadcast<
    float, RowMajor,
...
static int run_impl(
    ...
    cudaStream_t stream)
{
    using Gemm = GemmF32SimtBiasRrrSm80<ActivationOp>;
...
namespace bs = baracuda_cutlass::bias_f32_simt_rrr;

#define BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES(SUFFIX, ACTIVATION_OP)                              \
    extern "C" int baracuda_cutlass_gemm_##SUFFIX##_f32_simt_rrr_sm80_run(                         \
        int m, int n, int k,                                                                       \
        const void* a, int64_t lda,                                                                \
        const void* b, int64_t ldb,                                                                \
        const void* c, int64_t ldc,                                                                \
    ...
    {                                                                                              \
        return bs::run_impl<ACTIVATION_OP>(                                                        \
            ...
            static_cast<cudaStream_t>(stream));                                                    \
                                                                                                   ...
                                                                                                   \
    extern "C" std::size_t baracuda_cutlass_gemm_##SUFFIX##_f32_simt_rrr_sm80_workspace_size(      \
        int m, int n, int k)                                                                       \
    {                                                                                              \
        return bs::workspace_impl<ACTIVATION_OP>(m, n, k);                                         \
    }                                                                                              \
                                                                                                   \
    extern "C" int baracuda_cutlass_gemm_##SUFFIX##_f32_simt_rrr_sm80_can_implement(               \
        int m, int n, int k,                                                                       \
        const void* a, int64_t lda,                                                                \
        const void* b, int64_t ldb,                                                                \
        const void* c, int64_t ldc,                                                                \
...
BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES(bias,      bs::ActIdentity)
BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES(bias_relu, bs::ActReLu)
BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES(bias_gelu, bs::ActGELU)
BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES(bias_silu, bs::ActSiLu)

#undef BARACUDA_F32SIMT_BIAS_RRR_GEMM_ENTRIES
