// Same as probe.cu but adds head-to-head cuBLAS timing.
#include <cuda.h>
...
// The 128_4096_4096 kernel template (BM=64, BN=128, BK=64, Stages=3).
...
__global__ void cuda_l2_3090_kernel_128(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
void launch_128(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
  auto BM = Int<64>{};
  auto BN = Int<128>{};
  auto BK = Int<64>{};
  ...
  cudaFuncSetAttribute(cuda_l2_3090_kernel_128<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
  cuda_l2_3090_kernel_128<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}

int main(int argc, char** argv) {
  const int M = 128, N = 4096, K = 4096;

  std::vector<__half> h_a(M*K), h_b_kn(K*N), h_b_nk(N*K), h_c(M*N, __half(0.0f));
  for (int i = 0; i < M*K; ++i) h_a[i] = __float2half(0.01f);
  for (int i = 0; i < K*N; ++i) h_b_kn[i] = __float2half(0.01f);
  for (int i = 0; i < N*K; ++i) h_b_nk[i] = __float2half(0.01f);
  ...
  cudaMemcpy(d_b_kn, h_b_kn.data(), sizeof(__half)*K*N, cudaMemcpyHostToDevice);
  cudaMemcpy(d_b_nk, h_b_nk.data(), sizeof(__half)*N*K, cudaMemcpyHostToDevice);

  launch_128<__half, 3, true>(d_a, d_b_nk, d_c, M, N, K, 2048);
  cudaDeviceSynchronize();
  cudaMemcpy(h_c.data(), d_c, sizeof(__half)*M*N, cudaMemcpyDeviceToHost);
  printf("CUDA-L2 M=128 verify C[0]=%f (expected ~0.4096)\n", __half2float(h_c[0]));

  cudaEvent_t s, e;
  cudaEventCreate(&s); cudaEventCreate(&e);
  for (int i = 0; i < 10; ++i) launch_128<__half, 3, true>(d_a, d_b_nk, d_c, M, N, K, 2048);
  cudaDeviceSynchronize();
  cudaEventRecord(s);
  const int reps = 100;
  for (int i = 0; i < reps; ++i) launch_128<__half, 3, true>(d_a, d_b_nk, d_c, M, N, K, 2048);
  cudaEventRecord(e);
  cudaEventSynchronize(e);
  float ms = 0;
  cudaEventElapsedTime(&ms, s, e);
  float l2_us = ms * 1000.0f / reps;
  printf("CUDA-L2 3090 M128_N4096_K4096 fp16/fp32acc: %.2f us/iter\n", l2_us);
  ...
  printf("cuBLAS    M128_N4096_K4096 fp16/fp32acc: %.2f us/iter\n", cublas_us);

  printf("\n=== ratio (CUDA-L2 / cuBLAS) = %.3f ===\n", l2_us / cublas_us);
  if (l2_us < cublas_us) {
    printf("CUDA-L2 WINS (%.1f%% faster than cuBLAS)\n", 100.0f * (cublas_us - l2_us) / cublas_us);
  } else {
    printf("cuBLAS WINS (%.1f%% faster than CUDA-L2)\n", 100.0f * (l2_us - cublas_us) / l2_us);
  }
