// Wrapper that exposes the CUDA-L2 128_4096_4096 kernel under a
// stable C ABI symbol for the Rust bench harness. Strips the torch::
// pybind tail by `#define`-ing torch types to no-ops.
//
// We can't simply `#include` the upstream file because (a) it pulls
// in <torch/extension.h> at the bottom; (b) the kernel + launcher
// templates are at global scope and would collide if we tried to
// include multiple shape files in one TU. Each shape variant gets
// its own wrapper .cu compiled as a separate TU; the C ABI launcher
// here is the only externally-visible symbol.
//
// We inline the kernel + launcher body directly (instead of
// `#include`-ing the upstream .cu) for two reasons:
//   1. The upstream file has a torch::Tensor binding glued at the
//      bottom that pulls in libtorch headers we don't have.
//   2. Each shape file declares the kernel/launcher with the SAME
//      template names — including more than one in the bench would
//      ODR-clash. Putting each into its own TU + giving them
//      unique-suffixed C ABI symbols sidesteps that.
//
// MIT license preserved from upstream `kernels/3090_F32F16F16F32/
// 128_4096_4096.cu`. © 2025 deepreinforce.ai. See LICENSE.txt at the
// CUDA-L2 vendor root.
...
namespace baracuda_cuda_l2_m128 {
...
__global__ void kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
void launch(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride, cudaStream_t stream) {
  using namespace cute;
  auto BM = Int<64>{};
  auto BN = Int<128>{};
  auto BK = Int<64>{};
  ...
  cudaFuncSetAttribute(kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
  kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle><<<grid, block, kShmSize, stream>>>(a, b, c, M, N, K);
}

} // namespace baracuda_cuda_l2_m128

// C ABI launcher exposed to Rust. Operands match the upstream contract:
// `a` row-major (M, K), `b_col_major` col-major (K, N) [== row-major
// transpose (N, K)], `c` row-major (M, N). f16 only. fp32 accumulator
// internal.
extern "C" void baracuda_cuda_l2_m128_launch(const __half *a, const __half *b_col_major,
                                              __half *c, int M, int N, int K,
                                              cudaStream_t stream) {
  baracuda_cuda_l2_m128::launch<__half, 3, true>(
      const_cast<__half *>(a), const_cast<__half *>(b_col_major), c, M, N, K, 2048, stream);
}
