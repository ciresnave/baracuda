// Minimal standalone probe to verify CUDA-L2's 3090 HGEMM kernel
// builds + runs on sm_89 (RTX 4070). Strips the torch::Tensor binding
// and calls the launch_hgemm_mma_stages_block_swizzle_tn_cute template
// directly with raw half* pointers.
//
// Built with:
...
//        -I C:/Users/cires/.baracuda-cutlass-sys/checkouts/cutlass-4_2_0/include \
//        -I C:/Users/cires/.baracuda-cutlass-sys/checkouts/cutlass-4_2_0/tools/util/include \
//        probe.cu -o probe.exe
...
// Include the kernel template + launcher (skip the torch:: tail by
// reading from the source up to the launch function close brace).
// We #include the file but #define torch::Tensor so the torch
// bindings parse-skip — except we can't easily do that. Easier:
// copy the kernel body in directly.
...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m,
                                          int n, int k) {
...
int main() {
  ...
  std::vector<__half> h_a(M*K), h_b(N*K), h_c(M*N, __half(0.0f));
  ...
  cudaMemcpy(d_b, h_b.data(), sizeof(__half)*N*K, cudaMemcpyHostToDevice);
  cudaMemcpy(d_c, h_c.data(), sizeof(__half)*M*N, cudaMemcpyHostToDevice);

  launch_hgemm_mma_stages_block_swizzle_tn_cute<__half, 3, true>(
      d_a, d_b, d_c, M, N, K, 2048);
  ...
  printf("CUDA-L2 3090 128x4096x4096 fp16/fp32acc: %.2f us/iter\n", ms * 1000.0f / reps);

  cudaMemcpy(h_c.data(), d_c, sizeof(__half)*M*N, cudaMemcpyDeviceToHost);
  printf("C[0] = %f, C[M*N-1] = %f\n", __half2float(h_c[0]), __half2float(h_c[M*N-1]));
  // Expected: K * 0.01 * 0.01 = 0.4096
  if (fabsf(__half2float(h_c[0]) - 0.4096f) > 0.01f) {
    fprintf(stderr, "WRONG: expected ~0.4096, got %f\n", __half2float(h_c[0]));
    return 2;
  }
  printf("OK\n");
  return 0;
