        ...
        for &hidden in CROSS_HIDDEN_SWEEP {
            ...
            append_csv_row(
                ...
                &PhaseTwentyNineRow {
                    ...
                    reference_ns: None,
                    reference: "",
                    pytorch_ns: baseline.and_then(|b| b.lookup("layernorm", &shape, dtype_label)),
                },
            );
...
fn ln_benches(c: &mut Criterion) {
    ...
    bench_baracuda::<f32>(c, "f32", 1.0_f32, baseline_ref);
    bench_baracuda::<f16>(c, "f16", f16::ONE, baseline_ref);
}
