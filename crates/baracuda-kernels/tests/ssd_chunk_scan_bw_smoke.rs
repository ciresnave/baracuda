...
use baracuda_kernels::{
    ElementKind, PlanPreference, SsdChunkScanArgs, SsdChunkScanBackwardArgs,
    SsdChunkScanBackwardDescriptor, SsdChunkScanBackwardPlan, SsdChunkScanDescriptor,
    SsdChunkScanPlan, TensorMut, TensorRef, Workspace, contiguous_stride,
};
...
fn run_fw_and_sum(
    ...
    let plan =
        SsdChunkScanPlan::<f32>::select(stream, &desc, PlanPreference::default()).expect("select");
...
fn ssd_bw_dx_matches_fd_and_others_finite() {
    ...
    let b_host: Vec<f32> = (0..B * L * H * N)
        .map(|i| ((i as f32) * 0.07).sin() * 0.3)
        .collect();
    let c_host: Vec<f32> = (0..B * L * H * N)
    ...
    let bw_plan =
        SsdChunkScanBackwardPlan::<f32>::select(&stream, &bw_desc, PlanPreference::default())
            .expect("bw select");
