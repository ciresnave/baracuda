//! Real-GPU smoke test for unary Acos backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Acos`). Backward:
...
fn acos_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Acos, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "acos bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn acos_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Acos, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "acos bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn acos_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Acos, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "acos bw bf16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn acos_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Acos, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "acos bw f64 @ {i}");
    }
}
