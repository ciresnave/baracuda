...
use baracuda_kernels::{
    ElementKind, MaxPool3dPlan, PlanPreference, Pool3dBwArgs, Pool3dDescriptor, Pool3dFwArgs,
...
fn max_pool3d_f32() {
    ...
    let plan =
        MaxPool3dPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    ...
    assert!(
        (got_y[0] - exp_y[0]).abs() <= 16.0 * f32::EPSILON,
        "max_pool3d f32 y: got={} want={}",
        got_y[0],
        exp_y[0]
    );
