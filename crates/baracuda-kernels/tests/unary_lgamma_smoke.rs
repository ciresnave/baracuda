...
//! `UnaryPlan<T, N> + UnaryKind::Lgamma` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - `lgammaf` / `lgamma` libdevice routines may diverge from Rust's `libm` by ~1 ULP. Domain: strictly positive in this smoke test.
//!
//! Input range: `[0.5, ~10]`. Steered well clear of `0` and negative integers (poles of the Gamma function).
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_lgamma_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { libm::lgammaf(x) }
fn cpu_ref_f64(x: f64) -> f64 { libm::lgamma(x) }

fn sample_f32(i: usize) -> f32 { ((i % 200) as f32) * 0.05 + 0.5 }
fn sample_f64(i: usize) -> f64 { ((i % 200) as f64) * 0.05 + 0.5 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "lgamma f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "lgamma f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "lgamma f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "lgamma bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("lgamma f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("lgamma f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("lgamma bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("lgamma f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn lgamma_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
