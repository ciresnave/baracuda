//! Real-GPU smoke test for unary Asinh backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Asinh`). Backward:
//! `dx = dy / sqrt(1 + x²)`. Saved-x. Domain: all real x.
...
fn asinh_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Asinh, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "asinh bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn asinh_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Asinh, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "asinh bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn asinh_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Asinh, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "asinh bw bf16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn asinh_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Asinh, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "asinh bw f64 @ {i}");
    }
}
