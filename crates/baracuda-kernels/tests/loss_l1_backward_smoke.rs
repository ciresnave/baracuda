//! Real-GPU smoke test for `L1LossBackwardPlan`. BW × 4 dtypes × Mean.
//!
...
fn host_l1_bw_f64(pred: &[f64], target: &[f64], dy: f64, n: usize) -> Vec<f64> {
...
fn loss_l1_backward_f32_mean() {
    ...
    let dy_host = [1.0f32];
    let expected: Vec<f32> = host_l1_bw_f64(
    ...
    let desc = L1LossBackwardDescriptor {
        input_shape: shape,
        reduction: LossReduction::Mean,
        element: ElementKind::F32,
    };
    let plan =
        L1LossBackwardPlan::<f32, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::None,
        L1LossBackwardArgs {
...
#[test]
#[ignore]
fn loss_l1_backward_f64_mean() {
    let (ctx, stream) = setup();
    let shape = [2i32, 6];
    let numel = 12usize;
    let host_p: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.05 - 0.2).collect();
    let host_t: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.06 - 0.1).collect();
    let dy_host = [2.0f64];
    let expected = host_l1_bw_f64(&host_p, &host_t, 2.0, numel);
    ...
    let plan =
        L1LossBackwardPlan::<f64, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::None,
        L1LossBackwardArgs {
...
fn loss_l1_backward_f16_mean() {
    ...
    let dy_host_f32 = [1.0f32];
    let dy_host: Vec<f16> = dy_host_f32.iter().map(|&v| f16::from_f32(v)).collect();
    let p64: Vec<f64> = host_p.iter().map(|&v| v.to_f32() as f64).collect();
    let t64: Vec<f64> = host_t.iter().map(|&v| v.to_f32() as f64).collect();
    let expected_f64 = host_l1_bw_f64(&p64, &t64, 1.0, numel);
    ...
    let plan =
        L1LossBackwardPlan::<f16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::None,
        L1LossBackwardArgs {
...
fn loss_l1_backward_bf16_mean() {
    ...
    let dy_host_f32 = [1.0f32];
    let dy_host: Vec<bf16> = dy_host_f32.iter().map(|&v| bf16::from_f32(v)).collect();
    let p64: Vec<f64> = host_p.iter().map(|&v| v.to_f32() as f64).collect();
    let t64: Vec<f64> = host_t.iter().map(|&v| v.to_f32() as f64).collect();
    let expected_f64 = host_l1_bw_f64(&p64, &t64, 1.0, numel);
    ...
    let plan =
        L1LossBackwardPlan::<bf16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::None,
        L1LossBackwardArgs {
