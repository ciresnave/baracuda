//! Real-GPU smoke test for unary Reciprocal backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Reciprocal`).
...
fn reciprocal_backward_f64_3d() {
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "reciprocal bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn reciprocal_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
...
#[test]
#[ignore]
fn reciprocal_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
