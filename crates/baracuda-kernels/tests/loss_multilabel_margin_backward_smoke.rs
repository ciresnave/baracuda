...
fn loss_multilabel_margin_backward_f32_mean() {
    ...
    let plan =
        MultilabelMarginLossBackwardPlan::<f32>::select(&stream, &desc, PlanPreference::default())
            .unwrap();
    plan.run(
        &stream,
        Workspace::None,
...
fn loss_multilabel_margin_backward_f64_mean() {
    ...
    let plan =
        MultilabelMarginLossBackwardPlan::<f64>::select(&stream, &desc, PlanPreference::default())
            .unwrap();
    plan.run(
        &stream,
        Workspace::None,
...
fn loss_multilabel_margin_backward_f16_mean() {
    ...
    let plan =
        MultilabelMarginLossBackwardPlan::<f16>::select(&stream, &desc, PlanPreference::default())
            .unwrap();
    plan.run(
        &stream,
        Workspace::None,
...
fn loss_multilabel_margin_backward_bf16_mean() {
    ...
    let plan =
        MultilabelMarginLossBackwardPlan::<bf16>::select(&stream, &desc, PlanPreference::default())
            .unwrap();
    plan.run(
        &stream,
        Workspace::None,
