...
fn host_l1_mean_f64(pred: &[f64], target: &[f64]) -> f64 {
    let mut s = 0.0;
    for i in 0..pred.len() {
        s += (pred[i] - target[i]).abs();
    }
    s / (pred.len() as f64)
}
...
fn loss_l1_f32_mean() {
    ...
    let desc = L1LossDescriptor {
        input_shape: shape,
        reduction: LossReduction::Mean,
        element: ElementKind::F32,
    };
    let plan = L1LossPlan::<f32, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
...
fn loss_l1_f64_mean() {
    ...
    let shape = [4i32, 4];
    let numel = 16usize;
    let host_p: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.1 - 0.7).collect();
    let host_t: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.07).collect();
    let expected = host_l1_mean_f64(&host_p, &host_t);
    ...
    let plan = L1LossPlan::<f64, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        L1LossArgs {
...
fn loss_l1_f16_mean() {
    ...
    let expected = host_l1_mean_f64(&p64, &t64) as f32;
    ...
    let plan = L1LossPlan::<f16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        L1LossArgs {
...
fn loss_l1_bf16_mean() {
    ...
    let plan = L1LossPlan::<bf16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        L1LossArgs {
