//! Real-GPU smoke test for `BinaryCmpPlan<T, N> + BinaryCmpKind::Le`
...
//! larger) so both true outcomes and the false outcome are covered.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \
//!   --test binary_cmp_le_smoke -- --ignored`.
...
fn cmp_le_f32_3d() {
    ...
    let desc = BinaryCmpDescriptor { kind: BinaryCmpKind::Le, shape, element: ElementKind::F32 };
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp le f32 mismatch @ {i}");
    }
...
fn cmp_le_f16_3d() {
    ...
    let mut dev_y: DeviceBuffer<u8> = DeviceBuffer::zeros(&ctx, numel).expect("alloc y");
    let stride = contiguous_stride(shape);
    let desc = BinaryCmpDescriptor { kind: BinaryCmpKind::Le, shape, element: ElementKind::F16 };
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp le f16 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
fn cmp_le_bf16_3d() {
    ...
    let mut dev_y: DeviceBuffer<u8> = DeviceBuffer::zeros(&ctx, numel).expect("alloc y");
    let stride = contiguous_stride(shape);
    let desc = BinaryCmpDescriptor { kind: BinaryCmpKind::Le, shape, element: ElementKind::Bf16 };
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp le bf16 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
fn cmp_le_f64_3d() {
    ...
    let mut dev_y: DeviceBuffer<u8> = DeviceBuffer::zeros(&ctx, numel).expect("alloc y");
    let stride = contiguous_stride(shape);
    let desc = BinaryCmpDescriptor { kind: BinaryCmpKind::Le, shape, element: ElementKind::F64 };
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp le f64 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
#[test]
#[ignore]
fn cmp_le_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    let desc = BinaryCmpDescriptor { kind: BinaryCmpKind::Le, shape: y_shape, element: ElementKind::F32 };
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp le f32 strided mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
