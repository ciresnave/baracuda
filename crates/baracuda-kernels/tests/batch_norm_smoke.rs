...
fn batch_norm_f32_with_affine() {
    ...
    let plan =
        BatchNormPlan::<f32, 4>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    plan.run(
        &stream,
        Workspace::None,
        BatchNormArgs {
...
fn batch_norm_f32_no_affine() {
    ...
    let (exp_y, _, _) = host_batch_norm_f32(n as usize, c as usize, s, &host_x, None, None, eps);
...
fn batch_norm_f64_with_affine() {
    ...
    let plan =
        BatchNormPlan::<f64, 4>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    plan.run(
        &stream,
        Workspace::None,
        BatchNormArgs {
...
fn batch_norm_f16_with_affine() {
    ...
    let plan =
        BatchNormPlan::<f16, 4>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    plan.run(
        &stream,
        Workspace::None,
        BatchNormArgs {
...
fn batch_norm_bf16_with_affine() {
    ...
    let plan =
        BatchNormPlan::<bf16, 4>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    plan.run(
        &stream,
        Workspace::None,
        BatchNormArgs {
