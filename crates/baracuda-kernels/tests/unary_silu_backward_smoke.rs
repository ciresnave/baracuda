//! Real-GPU smoke test for unary SiLU (Swish) backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Silu`).
//!
//! Forward: `y = x * sigmoid(x)`. Backward: `dx = dy * s * (1 + x*(1-s))`
//! with `s = sigmoid(x) = 1/(1+exp(-x))`. Saved-x.
//!
//! Tolerance model: **cancellation-weighted absolute**. For `x` around
//! -1.5 the bracket `1 + x*(1-s)` has ~7× cancellation (e.g. at x=-1.44,
//! 1 + (-1.16) = -0.16). We bound the error by `|dy|·s·(1 + |x·(1-s)|)`,
//! the textbook cancellation magnitude. K = 16 absorbs the libdevice-
//! vs-libm gap for `expf` × the multiplicative chain.
...
fn silu_bw_f32(dy: f32, x: f32) -> (f32, f32) {
    let s = 1.0 / (1.0 + (-x).exp());
    let result = dy * s * (1.0 + x * (1.0 - s));
    let cancel_mag = dy.abs() * s * (1.0 + (x * (1.0 - s)).abs());
    (result, cancel_mag)
}
fn silu_bw_f64(dy: f64, x: f64) -> (f64, f64) {
    let s = 1.0 / (1.0 + (-x).exp());
    let result = dy * s * (1.0 + x * (1.0 - s));
    let cancel_mag = dy.abs() * s * (1.0 + (x * (1.0 - s)).abs());
    (result, cancel_mag)
...
fn silu_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Silu, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let (exp, cancel_mag) = silu_bw_f32(host_dy[i], host_x[i]);
        let tol = (cancel_mag.max(exp.abs())) * 16.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "silu bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn silu_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Silu, shape, element: ElementKind::F64 };
    ...
    dev_dx.copy_to_host(&mut got).expect("download");
    for i in 0..numel {
        let (exp, cancel_mag) = silu_bw_f64(host_dy[i], host_x[i]);
        let tol = (cancel_mag.max(exp.abs())) * 16.0 * f64::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "silu bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn silu_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Silu, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let (exp, cancel_mag) = silu_bw_f32(dy, fx);
        let g = got[i].to_f32();
        let tol = (cancel_mag.max(exp.abs())) * 16.0 * F16_EPS;
        assert!((g - exp).abs() <= tol, "silu bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn silu_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Silu, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "silu bw bf16 @ {i}: got {g} exp {exp}");
    }
}
