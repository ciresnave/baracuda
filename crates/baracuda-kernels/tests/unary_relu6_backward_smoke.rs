//! Real-GPU smoke test for unary ReLU6 backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Relu6`).
//!
//! Forward: `y = clamp(x, 0, 6)`. Backward: `dx = (0 < x < 6) ? dy : 0`.
//! Saved-x; piecewise activation BW.
//!
//! Bit-exact compare for every dtype — kernel is compare + select.
...
#[test]
#[ignore]
fn relu6_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    // Span [-2, ~10): straddles both 0 and 6.
    let host_x: Vec<f32> = (0..numel).map(|i| (i as f32) * 0.0001 - 2.0).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Relu6, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = if host_x[i] > 0.0 && host_x[i] < 6.0 { host_dy[i] } else { 0.0 };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "relu6 bw f32 @ {i}");
    }
}

#[test]
#[ignore]
fn relu6_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.0001 - 2.0).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Relu6, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        let exp = if host_x[i] > 0.0 && host_x[i] < 6.0 { host_dy[i] } else { 0.0 };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "relu6 bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn relu6_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32((i as f32) * 0.0001 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Relu6, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp = if fx > 0.0 && fx < 6.0 { host_dy[i] } else { f16::from_f32(0.0) };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "relu6 bw f16 @ {i}");
    }
}

#[test]
#[ignore]
fn relu6_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32((i as f32) * 0.0001 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Relu6, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        let fx = host_x[i].to_f32();
        let exp = if fx > 0.0 && fx < 6.0 { host_dy[i] } else { bf16::from_f32(0.0) };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "relu6 bw bf16 @ {i}");
    }
}
