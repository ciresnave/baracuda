...
//! `UnaryPlan<T, N> + UnaryKind::Tanh` across f32 / f16 / bf16 / f64.
//!
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanh f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanh f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanh f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanh bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let expected: Vec<f32> = host_x.iter().map(|x| (*x).tanh()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("tanh f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("tanh f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<bf16> = host_x
        .iter()
        .map(|x| bf16::from_f32(x.to_f32().tanh()))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("tanh bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let expected: Vec<f64> = host_x.iter().map(|x| (*x).tanh()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("tanh f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn tanh_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            let x_lin = j * M + i;
            let y_lin = i * N_DIM + j;
            expected[y_lin] = x_buf[x_lin].tanh();
        }
    }
