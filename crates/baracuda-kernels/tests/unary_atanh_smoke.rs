...
//! `UnaryPlan<T, N> + UnaryKind::Atanh` across f32 / f16 / bf16 / f64.
//!
...
//!   `--test unary_atanh_smoke -- --ignored`.
...
// 1-ULP relative tolerance constants (matches sqrt smoke test scheme).
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "atanh f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "atanh f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "atanh f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "atanh bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let host_x: Vec<f32> = (0..numel).map(|i| (((i % 200) as f32) * 0.009 - 0.9)).collect();
    let expected: Vec<f32> = host_x.iter().map(|x| (*x).atanh()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("atanh f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 200) as f32) * 0.009 - 0.9))
        .collect();
    let host_expected: Vec<f16> = host_x
        .iter()
        .map(|x| f16::from_f32({ let x = x.to_f32().atanh(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("atanh f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 200) as f32) * 0.009 - 0.9))
        .collect();
    let host_expected: Vec<bf16> = host_x
        .iter()
        .map(|x| bf16::from_f32({ let x = x.to_f32().atanh(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("atanh bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<f64> = (0..numel).map(|i| (((i % 200) as f64) * 0.009 - 0.9)).collect();
    let expected: Vec<f64> = host_x.iter().map(|x| (*x).atanh()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("atanh f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn atanh_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    let x_buf: Vec<f32> = (0..(N_DIM * M)).map(|i| (((i % 200) as f32) * 0.009 - 0.9)).collect();
    let x_shape = [m, n];
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            let x_lin = j * M + i;
            let y_lin = i * N_DIM + j;
            expected[y_lin] = { let x = x_buf[x_lin].atanh(); x };
        }
    }
