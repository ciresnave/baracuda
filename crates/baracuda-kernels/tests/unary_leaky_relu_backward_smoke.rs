//! Real-GPU smoke test for `UnaryBackwardPlan + UnaryKind::LeakyRelu`.
//!
//! Forward: `y = x if x > 0 else α·x`. Backward:
//! `dx = dy if x > 0 else dy·α`. α hardcoded to 0.01.
//!
//! f32 / f64 use a 4·eps relative-tolerance compare — the negative branch
...
#[test]
#[ignore]
fn leaky_relu_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
