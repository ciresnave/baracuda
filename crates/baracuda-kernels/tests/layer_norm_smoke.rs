...
use baracuda_kernels::{
    ElementKind, LayerNormArgs, LayerNormDescriptor, LayerNormPlan, PlanPreference, TensorMut,
...
fn host_layer_norm_f32(
    ...
    for outer_lin in 0..outer_numel {
        ...
        for d in 0..n {
            off_save += coord[d] as i64 * outer_stride[d];
        }
        // mean
        let mut sum = 0f64;
        for j in 0..extent {
...
fn layer_norm_f32_full_affine() {
    ...
    let host_x: Vec<f32> = (0..numel)
        .map(|i| ((i as f32) * 0.12 - 1.3).sin() * 1.4)
        .collect();
    ...
    let mut dev_mean: DeviceBuffer<f32> =
        DeviceBuffer::zeros(&ctx, expected_mean.len()).expect("alloc mean");
    let mut dev_std: DeviceBuffer<f32> =
        DeviceBuffer::zeros(&ctx, expected_std.len()).expect("alloc std");
    ...
    let plan =
        LayerNormPlan::<f32, 3>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    ...
    plan.run(
        ...
        LayerNormArgs {
            ...
            mean: TensorMut {
                ...
                shape: save_shape,
                stride: contiguous_stride(save_shape),
            },
            inv_std: TensorMut {
                data: dev_std.as_slice_mut(),
                shape: save_shape,
                stride: contiguous_stride(save_shape),
            },
        },
...
fn layer_norm_f64_no_affine() {
    ...
    let host_x: Vec<f64> = (0..numel)
        .map(|i| ((i as f64) * 0.2 - 1.0).cos() * 0.9)
        .collect();
    let eps = 1e-6f32;
    // CPU f64 ref
    ...
    let plan =
        LayerNormPlan::<f64, 2>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let save_shape = desc.save_shape();
    plan.run(
        &stream,
        Workspace::None,
    ...
    for i in 0..3 {
        ...
        assert!(
            (got_mean[i] - expected_mean[i]).abs() <= tol,
            "f64 layer_norm mean @ {i}"
        );
        let tol2 = (expected_std[i].abs() * eps_tol).max(eps_tol);
        assert!(
            (got_std[i] - expected_std[i]).abs() <= tol2,
            "f64 layer_norm inv_std @ {i}"
        );
    }
}
...
fn layer_norm_f16_full_affine() {
    ...
    let eps = 1e-5f32;
    let (expected_y_f32, _, _) = host_layer_norm_f32(
        &shape,
        1,
        &host_x_f32,
        Some(&host_gamma_f32),
    ...
    let plan =
        LayerNormPlan::<f16, 2>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let save_shape = desc.save_shape();
    plan.run(
        &stream,
        Workspace::None,
...
fn layer_norm_bf16_no_affine() {
    ...
    let plan =
        LayerNormPlan::<bf16, 2>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let save_shape = desc.save_shape();
    plan.run(
        &stream,
        Workspace::None,
