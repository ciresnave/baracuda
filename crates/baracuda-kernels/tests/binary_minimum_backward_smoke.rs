...
fn minimum_backward_f16_3d() {
    ...
    for i in 0..numel {
        ...
        assert!(
            (gd - exp_da).abs() <= tol,
            "minimum bw f16 da @ {i}: got {gd}, exp {exp_da}"
        );
        let gdb = got_db[i].to_f32();
        let tol = exp_db.abs().max(1.0) * F16_EPS;
        assert!(
            (gdb - exp_db).abs() <= tol,
            "minimum bw f16 db @ {i}: got {gdb}, exp {exp_db}"
        );
    }
}
...
fn minimum_backward_bf16_3d() {
    ...
    for i in 0..numel {
        ...
        assert!(
            (gd - exp_da).abs() <= tol,
            "minimum bw bf16 da @ {i}: got {gd}, exp {exp_da}"
        );
        let gdb = got_db[i].to_f32();
        let tol = exp_db.abs().max(1.0) * BF16_EPS;
        assert!(
            (gdb - exp_db).abs() <= tol,
            "minimum bw bf16 db @ {i}: got {gdb}, exp {exp_db}"
        );
    }
}
