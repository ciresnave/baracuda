...
use baracuda_kernels::{
    ElementKind, LpPool1dBackwardPlan, LpPool1dBwArgs, LpPool1dDescriptor, LpPool1dFwArgs,
    LpPool1dPlan, LpPool2dBackwardPlan, LpPool2dBwArgs, LpPool2dDescriptor, LpPool2dFwArgs,
    LpPool2dPlan, PlanPreference, TensorMut, TensorRef, Workspace, contiguous_stride,
};
use half::f16;
...
fn lp_pool_p2_l2_2x2_kernel_f32() {
    ...
    let (exp_y, h_out, w_out) = host_lp_pool2d_fw(
        n as usize,
        c as usize,
        h_in as usize,
        w_in as usize,
    ...
    let desc = LpPool2dDescriptor::new(n, c, h_in, w_in, 2, 2, 2.0, ElementKind::F32);
    let plan = LpPool2dPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    ...
    for i in 0..exp_y.len() {
        ...
        assert!(
            ...
            "lp_pool2d p=2 f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
    }
...
fn lp_pool_p1_l1_2x2_kernel_f32() {
    ...
    let desc = LpPool2dDescriptor::new(n, c, h_in, w_in, 2, 2, 1.0, ElementKind::F32);
    let plan = LpPool2dPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let x_shape = [n, c, h_in, w_in];
    let y_shape = [n, c, h_out as i32, w_out as i32];
    plan.run_fw(
    ...
    for i in 0..exp_y.len() {
        ...
        assert!(
            (got_y[i] - exp_y[i]).abs() <= t,
            "lp_pool2d p=1 f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
    }
...
fn lp_pool_p3_2x2_kernel_f32() {
    ...
    let desc = LpPool2dDescriptor::new(n, c, h_in, w_in, 2, 2, p_val, ElementKind::F32);
    let plan = LpPool2dPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let x_shape = [n, c, h_in, w_in];
    let y_shape = [n, c, h_out as i32, w_out as i32];
    plan.run_fw(
    ...
    for i in 0..exp_y.len() {
        ...
        assert!(
            (got_y[i] - exp_y[i]).abs() <= t,
            "lp_pool2d p=3 f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
    }
...
fn lp_pool_1d_p2_f32() {
    ...
    let plan = LpPool1dPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    ...
    for i in 0..exp_y.len() {
        ...
        assert!(
            (got_y[i] - exp_y[i]).abs() <= t,
            "lp_pool1d p=2 f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
    }
...
fn lp_pool_bw_p2_2x2_f32() {
    ...
    let p_val = 2.0f32;
    let (host_y, h_out, w_out) = host_lp_pool2d_fw(
        n as usize,
        c as usize,
        h_in as usize,
        w_in as usize,
    ...
    let host_dy: Vec<f32> = (0..numel_y).map(|i| 0.25 + 0.125 * (i as f32)).collect();
    ...
    let desc = LpPool2dDescriptor::new(n, c, h_in, w_in, 2, 2, p_val, ElementKind::F32);
    let bw = LpPool2dBackwardPlan::<f32>::select(&stream, &desc, PlanPreference::default())
        .expect("bw sel");
    let x_shape = [n, c, h_in, w_in];
    let y_shape = [n, c, h_out as i32, w_out as i32];
    bw.run_bw(
    ...
    for i in 0..numel_x {
        ...
        assert!(
            ...
            "lp_pool2d_bw p=2 f32 dx @ {i}: got={} want={}",
            got_dx[i],
            exp_dx[i]
        );
    }
...
fn lp_pool_f16_p2_2x2() {
    ...
    let (exp_y_f32, h_out, w_out) = host_lp_pool2d_fw(
        n as usize,
        c as usize,
        h_in as usize,
        w_in as usize,
    ...
    let desc = LpPool2dDescriptor::new(n, c, h_in, w_in, 2, 2, 2.0, ElementKind::F16);
    let plan = LpPool2dPlan::<f16>::select(&stream, &desc, PlanPreference::default()).expect("sel");
    let x_shape = [n, c, h_in, w_in];
    let y_shape = [n, c, h_out as i32, w_out as i32];
    plan.run_fw(
        &stream,
    ...
    for i in 0..exp_y_f32.len() {
        ...
        assert!(
            (g - exp_y_f32[i]).abs() <= t,
            "lp_pool2d p=2 f16 y @ {i}: got={} want={}",
            g,
            exp_y_f32[i]
        );
    }
