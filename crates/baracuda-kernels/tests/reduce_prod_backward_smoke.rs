//! Real-GPU smoke test for reduce-prod backward
//! (`ReduceBackwardPlan<T, N> + ReduceKind::Prod`).
//!
//! Forward: `y = prod(x, dim=k)` (keepdim). Backward:
//! `dx[c] = dy[c_reduced] * y[c_reduced] / x[c]`. Needs BOTH saved
//! `x` (full shape) and saved `y` (keepdim shape). Caller must ensure
//! `x[c] != 0` per cell.
//!
//! Run with: `cargo test -p baracuda-kernels --release --features sm89 \
//!   --test reduce_prod_backward_smoke -- --ignored`.
...
// CPU reference: compute y = prod(x, axis), then dx[c] = dy[c_r] * y[c_r] / x[c].
fn host_prod_bw_f32(
    input_shape: [i32; 3],
    axis: usize,
    x: &[f32],
    ...
    for_each_coord::<3, _>(input_shape, |coord, x_linear| {
        let dy_lin = dy_index(coord, axis, dy_shape) as usize;
        dx[x_linear as usize] = dy[dy_lin] * y[dy_lin] / x[x_linear as usize];
    });
    (y, dx)
}
...
fn prod_backward_f32_3d_axis1() {
    ...
    // Keep x in [0.6, 1.4] so prod doesn't blow up to inf.
    let host_x: Vec<f32> = (0..dx_numel).map(|i| 0.6 + 0.04 * (i as f32)).collect();
    let host_dy: Vec<f32> = (0..dy_numel).map(|i| 0.5 + 0.25 * (i as f32)).collect();
    let (host_y, expected_dx) = host_prod_bw_f32(input_shape, axis, &host_x, &host_dy);
    ...
    for i in 0..dx_numel {
        ...
        assert!((got[i] - expected_dx[i]).abs() <= tol,
            "f32 prod BW @ {i}: got={} want={} diff={}",
            got[i], expected_dx[i], (got[i] - expected_dx[i]).abs());
    }
}

#[test]
#[ignore]
fn prod_backward_f64_3d_axis2() {
    let (ctx, stream) = setup();
    let input_shape = [3i32, 4, 5];
    let axis: usize = 2;
    let dx_numel: usize = input_shape.iter().map(|&d| d as usize).product();
    ...
    let host_x: Vec<f64> = (0..dx_numel).map(|i| 0.6 + 0.04 * (i as f64)).collect();
    let host_dy: Vec<f64> = (0..dy_numel).map(|i| 0.5 + 0.25 * (i as f64)).collect();

    let mut host_y = vec![1f64; dy_numel];
    for_each_coord::<3, _>(input_shape, |coord, x_linear| {
        let dy_lin = dy_index(coord, axis, dy_shape) as usize;
    ...
    for_each_coord::<3, _>(input_shape, |coord, x_linear| {
        let dy_lin = dy_index(coord, axis, dy_shape) as usize;
        expected_dx[x_linear as usize] = host_dy[dy_lin] * host_y[dy_lin] / host_x[x_linear as usize];
    });
    ...
    for i in 0..dx_numel {
        let tol = (expected_dx[i].abs() * eps).max(eps);
        assert!((got[i] - expected_dx[i]).abs() <= tol, "f64 prod BW @ {i}");
    }
}

#[test]
#[ignore]
fn prod_backward_f16_3d_axis0() {
    let (ctx, stream) = setup();
    let input_shape = [3i32, 4, 5];
    let axis: usize = 0;
    let dx_numel: usize = input_shape.iter().map(|&d| d as usize).product();
    ...
    let host_x_f32: Vec<f32> = (0..dx_numel).map(|i| 0.7 + 0.05 * (i as f32)).collect();
    let host_dy_f32: Vec<f32> = (0..dy_numel).map(|i| 0.5 + 0.25 * (i as f32)).collect();
    let (host_y_f32, expected_dx_f32) = host_prod_bw_f32(input_shape, axis, &host_x_f32, &host_dy_f32);
    ...
    dev_dx.copy_to_host(&mut got).expect("dl");
    let eps = 4.0 * 9.77e-4_f32; // 4 ULP at f16
    for i in 0..dx_numel {
        let tol = (expected_dx_f32[i].abs() * eps).max(eps);
        let diff = (got[i].to_f32() - expected_dx_f32[i]).abs();
        assert!(diff <= tol, "f16 prod BW @ {i}: got={} want={} diff={}",
            got[i].to_f32(), expected_dx_f32[i], diff);
    }
}

#[test]
#[ignore]
fn prod_backward_bf16_3d_axis1() {
    let (ctx, stream) = setup();
    let input_shape = [3i32, 4, 5];
    let axis: usize = 1;
    let dx_numel: usize = input_shape.iter().map(|&d| d as usize).product();
    ...
    dev_dx.copy_to_host(&mut got).expect("dl");
    let eps = 4.0 * 7.81e-3_f32; // 4 ULP at bf16
    for i in 0..dx_numel {
        let tol = (expected_dx_f32[i].abs() * eps).max(eps);
        let diff = (got[i].to_f32() - expected_dx_f32[i]).abs();
        assert!(diff <= tol, "bf16 prod BW @ {i}: diff={diff}");
    }
}
