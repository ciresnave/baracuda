...
fn mul_backward_f32_3d() {
    ...
    for i in 0..numel {
        ...
        assert_eq!(
            got_da[i].to_bits(),
            exp_da.to_bits(),
            "mul backward f32 da @ {i}"
        );
        assert_eq!(
            got_db[i].to_bits(),
            exp_db.to_bits(),
            "mul backward f32 db @ {i}"
        );
    }
}
...
fn mul_backward_f16_3d() {
    ...
    let host_a: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32((i % 37) as f32 * 0.125 + 1.0))
        .collect();
    let host_b: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32((i % 29) as f32 * 0.0625 - 1.5))
        .collect();
    let dev_dy = DeviceBuffer::from_slice(&ctx, &host_dy).expect("upload dy");
    let dev_a = DeviceBuffer::from_slice(&ctx, &host_a).expect("upload a");
    let dev_b = DeviceBuffer::from_slice(&ctx, &host_b).expect("upload b");
    ...
    for i in 0..numel {
        ...
        assert!(
            (gd - ed).abs() <= tol,
            "mul backward f16 da @ {i}: got {gd}, exp {ed}"
        );
        ...
        assert!(
            (gdb - edb).abs() <= tol,
            "mul backward f16 db @ {i}: got {gdb}, exp {edb}"
        );
    }
}
...
fn mul_backward_bf16_3d() {
    ...
    let host_a: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32((i % 37) as f32 * 0.125 + 1.0))
        .collect();
    let host_b: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32((i % 29) as f32 * 0.0625 - 1.5))
        .collect();
    let dev_dy = DeviceBuffer::from_slice(&ctx, &host_dy).expect("upload dy");
    let dev_a = DeviceBuffer::from_slice(&ctx, &host_a).expect("upload a");
    let dev_b = DeviceBuffer::from_slice(&ctx, &host_b).expect("upload b");
    ...
    for i in 0..numel {
        ...
        assert!(
            (gd - ed).abs() <= tol,
            "mul backward bf16 da @ {i}: got {gd}, exp {ed}"
        );
        let gdb = got_db[i].to_f32();
        let edb = exp_db.to_f32();
        ...
        assert!(
            (gdb - edb).abs() <= tol,
            "mul backward bf16 db @ {i}: got {gdb}, exp {edb}"
        );
    }
}
...
fn mul_backward_f64_3d() {
    ...
    for i in 0..numel {
        ...
        assert_eq!(
            got_da[i].to_bits(),
            exp_da.to_bits(),
            "mul backward f64 da @ {i}"
        );
        assert_eq!(
            got_db[i].to_bits(),
            exp_db.to_bits(),
            "mul backward f64 db @ {i}"
        );
    }
}
