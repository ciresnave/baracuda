...
//! `UnaryPlan<T, N> + UnaryKind::GeluTanh` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon — the activation composes from transcendentals
//! whose libdevice / Rust libm implementations may diverge by ~1 ULP.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_gelu_tanh_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { { let k = 0.7978845608028654_f32; let a = 0.044715_f32; 0.5_f32 * x * (1.0 + (k * (x + a * x * x * x)).tanh()) } }
fn cpu_ref_f64(x: f64) -> f64 { { let k = 0.7978845608028654_f64; let a = 0.044715_f64; 0.5_f64 * x * (1.0 + (k * (x + a * x * x * x)).tanh()) } }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "gelu_tanh f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "gelu_tanh f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "gelu_tanh f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "gelu_tanh bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let host_x: Vec<f32> = (0..numel).map(|i| ((i % 200) as f32) * 0.05 - 5.0).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("gelu_tanh f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("gelu_tanh f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("gelu_tanh bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("gelu_tanh f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn gelu_tanh_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    let x_buf: Vec<f32> = (0..(N_DIM * M)).map(|i| ((i % 200) as f32) * 0.05 - 5.0).collect();
    let x_shape = [m, n];
