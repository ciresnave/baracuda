...
//! `UnaryPlan<T, N> + UnaryKind::Tanhshrink` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - `tanhf` / `tanh` libdevice routines may diverge from Rust's `f32::tanh` / `f64::tanh` by ~1 ULP.
//!
//! Input range: `[-5, 5]`. For `|x| >= ~4`, `tanhshrink(x) approx x - sign(x)` so the dynamic range stays comfortable for half-precision.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_tanhshrink_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { { let f = x; f - f.tanh() } }
fn cpu_ref_f64(x: f64) -> f64 { { let f = x; f - f.tanh() } }

fn sample_f32(i: usize) -> f32 { (i as f32) * 0.05 - 5.0 }
fn sample_f64(i: usize) -> f64 { (i as f64) * 0.05 - 5.0 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanhshrink f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanhshrink f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanhshrink f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "tanhshrink bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("tanhshrink f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("tanhshrink f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("tanhshrink bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("tanhshrink f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn tanhshrink_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
...
#[test]
#[ignore]
fn tanhshrink_f32_3d() {
    run_f32_contig::<3>([8, 128, 128]);
}

#[test]
#[ignore]
fn tanhshrink_f16_3d() {
    run_f16_contig_3d([8, 128, 128]);
}

#[test]
#[ignore]
fn tanhshrink_bf16_3d() {
    run_bf16_contig_3d([8, 128, 128]);
}

#[test]
#[ignore]
fn tanhshrink_f64_3d() {
