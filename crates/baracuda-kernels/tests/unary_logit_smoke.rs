...
//! `UnaryPlan<T, N> + UnaryKind::Logit` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - `logf` / `log` libdevice routines may diverge from Rust's `f32::ln` / `f64::ln` by ~1 ULP. Domain: `(0, 1)`; at the endpoints `logit` produces +/- infinity.
//!
//! Input range: `[0.01, ~0.99]` - strictly inside the `(0, 1)` domain.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_logit_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { { let f = x; (f / (1.0 - f)).ln() } }
fn cpu_ref_f64(x: f64) -> f64 { { let f = x; (f / (1.0 - f)).ln() } }

fn sample_f32(i: usize) -> f32 { ((i % 200) as f32) * 0.0049 + 0.01 }
fn sample_f64(i: usize) -> f64 { ((i % 200) as f64) * 0.0049 + 0.01 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "logit f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "logit f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "logit f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "logit bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("logit f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("logit f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("logit bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("logit f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn logit_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
