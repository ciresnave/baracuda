...
#[test]
#[ignore]
fn segment_sum_backward_f32_basic() {
    ...
    let d_out: Vec<f32> = (0..(ns * d) as usize)
        ...
        .collect();
    // Expected: d_input[n, d] = d_output[seg[n], d].
    ...
    let desc = SegmentSumBackwardDescriptor {
        num_inputs: n,
        embedding_dim: d,
        num_segments: ns,
        element: ElementKind::F32,
    };
    let plan = SegmentSumBackwardPlan::<f32>::select(&stream, &desc, PlanPreference::default())
        .expect("select");
    let args = SegmentSumBackwardArgs::<f32> {
        d_output: TensorRef {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(
            g, e,
            "segment_sum_backward f32 mismatch @ {i}: got {g} expected {e}"
        );
    }
}
