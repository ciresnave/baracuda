//! Real-GPU smoke test for the gated GLU backward.
//!
...
fn glu_bw_ref_f32(dy: f32, a: f32, b: f32) -> (f32, f32) {
    let s = if b >= 0.0 { 1.0 / (1.0 + (-b).exp()) } else { let e = b.exp(); e / (1.0 + e) };
    (dy * s, dy * a * s * (1.0 - s))
}
fn glu_bw_ref_f64(dy: f64, a: f64, b: f64) -> (f64, f64) {
    let s = if b >= 0.0 { 1.0 / (1.0 + (-b).exp()) } else { let e = b.exp(); e / (1.0 + e) };
    (dy * s, dy * a * s * (1.0 - s))
}
...
fn cpu_ref_bw_f32(x: &[f32], dy: &[f32], input_shape: &[i32], split_dim: usize) -> Vec<f32> {
    ...
    for i in 0..out_numel {
        ...
        let (da, db) = glu_bw_ref_f32(dyv, a, b);
...
fn cpu_ref_bw_f64(x: &[f64], dy: &[f64], input_shape: &[i32], split_dim: usize) -> Vec<f64> {
    ...
    for i in 0..out_numel {
        ...
        let b = x[(off_x + x_half_offset) as usize];
        let dyv = dy[off_dy as usize];
        let (da, db) = glu_bw_ref_f64(dyv, a, b);
        dx[off_x as usize] = da;
        dx[(off_x + x_half_offset) as usize] = db;
    }
    dx
...
fn glu_bw_f32() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::Glu, input_shape, split_dim, element: ElementKind::F32 };
    ...
    for i in 0..in_numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "glu bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}
...
fn glu_bw_f64() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::Glu, input_shape, split_dim, element: ElementKind::F64 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "glu bw f64 @ {i}: got {} exp {}", got[i], exp);
    }
}
...
fn glu_bw_f16() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::Glu, input_shape, split_dim, element: ElementKind::F16 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        ...
        assert!((g - exp).abs() <= tol, "glu bw f16 @ {i}: got {g} exp {exp}");
    }
}
...
fn glu_bw_bf16() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::Glu, input_shape, split_dim, element: ElementKind::Bf16 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        ...
        assert!((g - exp).abs() <= tol, "glu bw bf16 @ {i}: got {g} exp {exp}");
    }
}
