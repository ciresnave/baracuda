...
#[test]
#[ignore]
fn loss_smooth_l1_backward_f32_mean() {
    ...
    let desc = SmoothL1LossBackwardDescriptor {
        input_shape: shape,
...
#[test]
#[ignore]
fn loss_smooth_l1_backward_f64_mean() {
    let (ctx, stream) = setup();
    let shape = [2i32, 6];
    let numel = 12usize;
    let beta = 0.5f32;
    ...
    let dy_host = [2.0f64];
    let expected = host_smooth_l1_bw_f64(&host_p, &host_t, 2.0, numel, beta as f64);
...
#[test]
#[ignore]
fn loss_smooth_l1_backward_f16_mean() {
    let (ctx, stream) = setup();
    let shape = [4i32, 4];
    let numel = 16usize;
    let beta = 1.0f32;
    ...
    let expected_f64 = host_smooth_l1_bw_f64(&p64, &t64, 1.0, numel, beta as f64);
...
#[test]
#[ignore]
fn loss_smooth_l1_backward_bf16_mean() {
    let (ctx, stream) = setup();
    let shape = [3i32, 5];
    let numel = 15usize;
    let beta = 1.0f32;
