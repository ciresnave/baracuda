...
#[test]
#[ignore]
fn rms_norm_f64_inplace_matches_non_aliased() {
    // Phase 65d-ext: f64 now routes through the SMEM-staged kernel via
    // the `block_reduce_sum_f64` reducer. Same in-place safety as f32.
    let (ctx, stream) = setup();
    let outer: usize = 4;
    let inner: usize = 128;
    let numel = outer * inner;
    ...
    let host_x: Vec<f64> = (0..numel)
        .map(|i| ((i as f64) * 0.013 - 1.5).cos() * 0.6)
        .collect();
    ...
    let mut dev_inplace = DeviceBuffer::from_slice(&ctx, &host_x).expect("up");
    let mut dev_rms_inplace: DeviceBuffer<f64> = DeviceBuffer::zeros(&ctx, outer).expect("alloc");
    let p = dev_inplace.as_slice_mut().as_raw().0;
    let status = unsafe {
        baracuda_kernels_sys::baracuda_kernels_rms_norm_f64_run(
            eps, numel as i64, 2, shape.as_ptr(),
