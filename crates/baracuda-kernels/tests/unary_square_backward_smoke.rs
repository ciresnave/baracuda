...
fn square_backward_f16_3d() {
    ...
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 100) as f32) * 0.04 - 2.0))
        .collect();
...
fn square_backward_bf16_3d() {
    ...
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 100) as f32) * 0.04 - 2.0))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert!(
            (g - exp).abs() <= tol,
            "square bw bf16 @ {i}: got {g} exp {exp}"
        );
    }
}
