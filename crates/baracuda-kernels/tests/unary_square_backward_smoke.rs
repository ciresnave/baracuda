//! Real-GPU smoke test for unary square backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Square`).
//! Backward: `dx = dy * 2 * x`. Saved-x, no transcendental.
...
fn square_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Square, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] * 2.0 * host_x[i];
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "square bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn square_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Square, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "square bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn square_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.04 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Square, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "square bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn square_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.04 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Square, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "square bw bf16 @ {i}: got {g} exp {exp}");
    }
}
