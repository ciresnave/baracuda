...
fn addcmul_backward_f32_3d() {
    ...
    for i in 0..numel {
        ...
        assert!(
            ...
            "addcmul BW f32 db @ {i}: got {} exp {}",
            got_db[i],
            ed_b
        );
        assert!(
            (got_dc[i] - ed_c).abs() <= tol_c,
            "addcmul BW f32 dc @ {i}: got {} exp {}",
            got_dc[i],
            ed_c
        );
    }
...
fn addcmul_backward_f16_3d() {
    ...
    let host_c: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 23) as f32) * 0.125 - 1.0))
        .collect();
    let dev_dy = DeviceBuffer::from_slice(&ctx, &host_dy).unwrap();
    let dev_a = DeviceBuffer::from_slice(&ctx, &host_a).unwrap();
    let dev_b = DeviceBuffer::from_slice(&ctx, &host_b).unwrap();
...
fn addcmul_backward_bf16_3d() {
    ...
    let host_c: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 23) as f32) * 0.125 - 1.0))
        .collect();
    let dev_dy = DeviceBuffer::from_slice(&ctx, &host_dy).unwrap();
    let dev_a = DeviceBuffer::from_slice(&ctx, &host_a).unwrap();
    let dev_b = DeviceBuffer::from_slice(&ctx, &host_b).unwrap();
...
fn addcmul_backward_f64_3d() {
    ...
    for i in 0..numel {
        ...
        assert!(
            (got_db[i] - ed_b).abs() <= tol_b,
            "addcmul BW f64 db @ {i}: got {} exp {}",
            got_db[i],
            ed_b
        );
        assert!(
            (got_dc[i] - ed_c).abs() <= tol_c,
            "addcmul BW f64 dc @ {i}: got {} exp {}",
            got_dc[i],
            ed_c
        );
    }
