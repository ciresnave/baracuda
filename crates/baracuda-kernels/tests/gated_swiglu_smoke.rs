//! Real-GPU smoke test for the gated SwiGLU forward
//! (`GatedActivationPlan<T, N> + GatedActivationKind::SwiGlu`).
//!
//! Forward: split `x` along `split_dim` into halves `(a, b)`,
//! output `y = a · silu(b) = a · b · sigmoid(b)`.
//!
//! Tolerances: 4·eps relative (f32 / f64), 4·F16_EPS / 4·BF16_EPS
//! (f16 / bf16) — sigmoid+exp inside.
//!
//! `#[ignore]` by default.
...
fn swiglu_fw_f32() {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!((g - e).abs() <= allow, "swiglu f32 @ {i}: got {g} exp {e}");
    }
}
...
fn swiglu_fw_f64() {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!((g - e).abs() <= allow, "swiglu f64 @ {i}: got {g} exp {e}");
    }
}
...
fn swiglu_fw_f16() {
    ...
    let expected_f32 = cpu_ref_swiglu_f32(&x_f32, &input_shape, split_dim as usize);
