...
//! `UnaryPlan<T, N> + UnaryKind::Ceil` across f32 / f16 / bf16 / f64.
//!
...
fn assert_eq_bits_f32(got: f32, expected: f32, idx: usize) {
    assert!(
        ...
        "ceil f32 mismatch @ {idx}: got {got} expected {expected}"
    );
}
...
fn assert_eq_bits_f64(got: f64, expected: f64, idx: usize) {
    assert!(
        got.to_bits() == expected.to_bits() || (got.is_nan() && expected.is_nan()),
        "ceil f64 mismatch @ {idx}: got {got} expected {expected}"
    );
}
...
fn assert_eq_bits_half(got_bits: u16, expected_bits: u16, idx: usize, label: &str) {
    assert!(
        ...
        "ceil {label} mismatch @ {idx}: got bits 0x{got_bits:04x} expected 0x{expected_bits:04x}"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let expected: Vec<f32> = host_x.iter().map(|x| (*x).ceil()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("ceil f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<f16> = host_x
        .iter()
        .map(|x| f16::from_f32({ let x = x.to_f32().ceil(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("ceil f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<bf16> = host_x
        .iter()
        .map(|x| bf16::from_f32({ let x = x.to_f32().ceil(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("ceil bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let expected: Vec<f64> = host_x.iter().map(|x| (*x).ceil()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("ceil f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn ceil_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            let x_lin = j * M + i;
            let y_lin = i * N_DIM + j;
            expected[y_lin] = { let x = x_buf[x_lin].ceil(); x };
        }
    }
