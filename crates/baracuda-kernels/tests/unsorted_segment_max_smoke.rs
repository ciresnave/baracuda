//! Real-GPU smoke test for `UnsortedSegmentMaxPlan<T>` (Phase 7 7.6).
//! `#[ignore]` by default.

use baracuda_driver::{Context, Device, DeviceBuffer, Stream, init};
use baracuda_kernels::{
    ElementKind, PlanPreference, TensorMut, TensorRef, UnsortedSegmentMaxArgs,
    UnsortedSegmentMaxDescriptor, UnsortedSegmentMaxPlan, Workspace, contiguous_stride,
};
...
#[test]
#[ignore]
fn unsorted_segment_max_f32_scrambled() {
    ...
    let desc = UnsortedSegmentMaxDescriptor {
        num_inputs: n,
        embedding_dim: d,
        num_segments: ns,
        element: ElementKind::F32,
    };
    let plan = UnsortedSegmentMaxPlan::<f32>::select(&stream, &desc, PlanPreference::default())
        .expect("select");
    let args = UnsortedSegmentMaxArgs::<f32> {
        input: TensorRef {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(
            g, e,
            "unsorted_segment_max f32 mismatch @ {i}: got {g} expected {e}"
        );
    }
}
