//! Real-GPU smoke test for unary Hardsigmoid backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Hardsigmoid`).
//!
//! Forward: `y = clamp(x/6 + 0.5, 0, 1)`. Backward (PyTorch convention):
//!   dx = (-3 < x < 3) ? dy / 6 : 0
//! Saved-x; piecewise + one scalar division.
//!
//! f32 / f64 use bit-exact compare against `dy / 6.0` host arithmetic
//! (the kernel performs the same division on the same dtype). f16 /
//! bf16 use 4×eps relative tolerance because the f32-detour does one
//! extra rounding on store.
...
#[test]
#[ignore]
fn hardsigmoid_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    // Span (-4, +4): straddles both ±3 boundaries.
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardsigmoid, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = if host_x[i] > -3.0 && host_x[i] < 3.0 { host_dy[i] / 6.0 } else { 0.0 };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardsigmoid bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn hardsigmoid_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardsigmoid, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        let exp = if host_x[i] > -3.0 && host_x[i] < 3.0 { host_dy[i] / 6.0 } else { 0.0 };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardsigmoid bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn hardsigmoid_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardsigmoid, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp = if fx > -3.0 && fx < 3.0 { dy / 6.0 } else { 0.0 };
        let g = got[i].to_f32();
        let tol = exp.abs().max(1.0) * 4.0 * F16_EPS;
        assert!((g - exp).abs() <= tol, "hardsigmoid bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn hardsigmoid_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardsigmoid, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "hardsigmoid bw bf16 @ {i}: got {g} exp {exp}");
    }
}
