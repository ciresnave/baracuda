...
fn hardsigmoid_backward_f32_3d() {
    ...
    for i in 0..numel {
        let exp = if host_x[i] > -3.0 && host_x[i] < 3.0 {
            host_dy[i] / 6.0
        } else {
            0.0
        };
...
fn hardsigmoid_backward_f16_3d() {
    ...
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 800) as f32) * 0.01 - 4.0))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert!(
            (g - exp).abs() <= tol,
            "hardsigmoid bw f16 @ {i}: got {g} exp {exp}"
        );
    }
}
...
fn hardsigmoid_backward_bf16_3d() {
    ...
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 800) as f32) * 0.01 - 4.0))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert!(
            (g - exp).abs() <= tol,
            "hardsigmoid bw bf16 @ {i}: got {g} exp {exp}"
        );
    }
}
