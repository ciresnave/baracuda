...
use baracuda_kernels::{
    ElementKind, GaussianNllLossArgs, GaussianNllLossDescriptor, GaussianNllLossPlan,
...
fn loss_gaussian_nll_f32_mean() {
    ...
    let plan =
        GaussianNllLossPlan::<f32, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
...
fn loss_gaussian_nll_f64_mean() {
    ...
    let plan =
        GaussianNllLossPlan::<f64, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        GaussianNllLossArgs {
...
fn loss_gaussian_nll_f16_mean() {
    ...
    let plan =
        GaussianNllLossPlan::<f16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        GaussianNllLossArgs {
...
fn loss_gaussian_nll_bf16_mean() {
    ...
    let plan =
        GaussianNllLossPlan::<bf16, 2>::select(&stream, &desc, PlanPreference::default()).unwrap();
    plan.run(
        &stream,
        Workspace::Borrowed(dev_ws.as_slice_mut()),
        GaussianNllLossArgs {
