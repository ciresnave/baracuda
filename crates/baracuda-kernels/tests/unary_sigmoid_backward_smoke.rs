...
fn sigmoid_backward_f16_3d() {
    ...
    let host_y: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 99) as f32) * 0.009 + 0.05))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert!(
            (g - exp).abs() <= tol,
            "sigmoid bw f16 @ {i}: got {g} exp {exp}"
        );
    }
}
...
fn sigmoid_backward_bf16_3d() {
    ...
    let host_y: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 99) as f32) * 0.009 + 0.05))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert!(
            (g - exp).abs() <= tol,
            "sigmoid bw bf16 @ {i}: got {g} exp {exp}"
        );
    }
}
