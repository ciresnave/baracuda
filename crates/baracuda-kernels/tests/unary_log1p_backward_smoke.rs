//! Real-GPU smoke test for unary log1p backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Log1p`).
//! Backward: `dx = dy / (1 + x)`. Saved-x.
...
#[test]
#[ignore]
fn log1p_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    // 1 + x must stay positive: keep x in [0, ~8).
    let host_x: Vec<f32> = (0..numel).map(|i| (i as f32) * 0.0625).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log1p, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] / (1.0 + host_x[i]);
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "log1p bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn log1p_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.0625).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log1p, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "log1p bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn log1p_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.0625)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log1p, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "log1p bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn log1p_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.0625)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log1p, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "log1p bw bf16 @ {i}: got {g} exp {exp}");
    }
}
