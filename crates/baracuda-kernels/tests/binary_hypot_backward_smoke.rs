//! Real-GPU smoke test for `BinaryBackwardPlan + BinaryKind::Hypot`.
//!
//! Forward: `y = sqrt(a² + b²)`. Backward:
//!   da = dy * a / y
//!   db = dy * b / y
//! The kernel reconstructs `y` from saved `a`/`b` inline (saved-y not
...
fn hypot_backward_bf16() {
    ...
    for i in 0..numel {
        ...
        let diff_da = (got_da[i].to_f32() - want_da).abs();
        let diff_db = (got_db[i].to_f32() - want_db).abs();
        assert!(diff_da <= tol_da, "bf16 hypot BW da @ {i}: diff={diff_da}");
        assert!(diff_db <= tol_db, "bf16 hypot BW db @ {i}: diff={diff_db}");
    }
}
