//! Real-GPU smoke test for `UnaryPlan + UnaryKind::Elu` across f32 /
//! f16 / bf16 / f64. α is hardcoded to 1.0 in the kernel (PyTorch
//! default).
//!
//! Inputs straddle zero. Tolerance: relative `4 * eps` — the negative
//! branch contains `exp(x) - 1` which composes from libdevice / Rust libm
//! transcendentals (may diverge by ~1 ULP).
...
fn cpu_ref_f32(x: f32) -> f32 { if x > 0.0 { x } else { x.exp() - 1.0 } }
fn cpu_ref_f64(x: f64) -> f64 { if x > 0.0 { x } else { x.exp() - 1.0 } }

fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    let diff = (got - expected).abs();
    let allow = expected.abs().max(1.0) * 4.0 * f32::EPSILON;
    assert!(diff <= allow, "elu f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})");
}

fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    let diff = (got - expected).abs();
    let allow = expected.abs().max(1.0) * 4.0 * f64::EPSILON;
    assert!(diff <= allow, "elu f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})");
}

fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    let diff = (got - expected).abs();
    let allow = expected.abs().max(1.0) * 4.0 * F16_EPS;
    assert!(diff <= allow, "elu f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})");
}

fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    let diff = (got - expected).abs();
    let allow = expected.abs().max(1.0) * 4.0 * BF16_EPS;
    assert!(diff <= allow, "elu bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})");
}
...
fn elu_f32_3d() {
    ...
    // Range ~[-4, 4] keeps exp(x) on f16/bf16 from overflowing.
    ...
    let desc = UnaryDescriptor { kind: UnaryKind::Elu, shape, element: ElementKind::F32 };
...
fn elu_f64_3d() {
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryDescriptor { kind: UnaryKind::Elu, shape, element: ElementKind::F64 };
...
fn elu_f16_3d() {
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryDescriptor { kind: UnaryKind::Elu, shape, element: ElementKind::F16 };
...
fn elu_bf16_3d() {
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryDescriptor { kind: UnaryKind::Elu, shape, element: ElementKind::Bf16 };
