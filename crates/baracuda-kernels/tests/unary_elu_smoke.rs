...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "elu f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "elu f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "elu f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "elu bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn elu_f16_3d() {
    ...
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 401) as f32) * 0.02 - 4.0))
        .collect();
...
fn elu_bf16_3d() {
    ...
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 401) as f32) * 0.02 - 4.0))
        .collect();
