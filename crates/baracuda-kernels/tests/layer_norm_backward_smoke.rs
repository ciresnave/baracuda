...
use baracuda_kernels::{
    ElementKind, LayerNormBackwardArgs, LayerNormBackwardDescriptor, LayerNormBackwardPlan,
...
fn host_layer_norm_bw_f32(
    ...
    let db_out = if has_beta {
        Some(dbeta.iter().map(|&v| v as f32).collect())
    } else {
        None
    };
    (dx, dg_out, db_out)
...
fn layer_norm_bw_f32_full_affine() {
    ...
    let host_x: Vec<f32> = (0..numel)
        .map(|i| ((i as f32) * 0.11 - 1.0).sin() * 1.2)
        .collect();
    ...
    let mut dev_dgamma: DeviceBuffer<f32> =
        DeviceBuffer::zeros(&ctx, extent).expect("alloc dgamma");
    ...
    plan.run(
        ...
        LayerNormBackwardArgs {
            ...
            mean: TensorRef {
                data: dev_mean.as_slice(),
                shape: save_shape,
                stride: contiguous_stride(save_shape),
            },
            inv_std: TensorRef {
                data: dev_std.as_slice(),
                shape: save_shape,
                stride: contiguous_stride(save_shape),
            },
            dx: TensorMut {
            ...
            dgamma: Some(TensorMut {
                data: dev_dgamma.as_slice_mut(),
                shape: [extent as i32],
                stride: [1],
            }),
            dbeta: Some(TensorMut {
                data: dev_dbeta.as_slice_mut(),
                shape: [extent as i32],
                stride: [1],
            }),
        },
    ...
    for i in 0..numel {
        ...
        assert!(
            (got_dx[i] - expected_dx[i]).abs() <= tol,
            "f32 layer_norm_bw dx @ {i}: got={} want={}",
            got_dx[i],
            expected_dx[i]
        );
    }
    ...
    for i in 0..extent {
        ...
        assert!(
            (got_dgamma[i] - expected_dgamma[i]).abs() <= tol,
            "f32 layer_norm_bw dgamma @ {i}"
        );
        let tol2 = (expected_dbeta[i].abs() * aff_tol).max(aff_tol);
        assert!(
            (got_dbeta[i] - expected_dbeta[i]).abs() <= tol2,
...
fn layer_norm_bw_f64_no_affine() {
    ...
    let host_dy: Vec<f64> = (0..numel)
        .map(|i| ((i as f64) * 0.13 + 0.5).sin() * 0.6)
        .collect();
    let eps = 1e-6f32;
    // CPU f64 ref
    ...
    for i in 0..numel {
        ...
        assert!(
            (got_dx[i] - expected_dx[i]).abs() <= tol,
            "f64 layer_norm_bw dx @ {i}: got={} want={}",
            got_dx[i],
            expected_dx[i]
        );
    }
...
fn layer_norm_bw_f16_full_affine() {
    ...
    let (expected_dx_f32, expected_dgamma_f32, expected_dbeta_f32) = host_layer_norm_bw_f32(
        &shape,
        1,
        &host_dy_f32,
        &host_x_f32,
