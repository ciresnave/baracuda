...
//! `UnaryPlan<T, N> + UnaryKind::Relu6` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - Piecewise-exact: `min` / `max` of arithmetic constants. No transcendentals; the 4 x eps tolerance is for uniformity with sibling smoke tests.
//!
//! Input range: `[-2, 12]`. Exercises all three pieces - the `< 0` clamp, the linear region, and the `> 6` clamp.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_relu6_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { x.max(0.0).min(6.0) }
fn cpu_ref_f64(x: f64) -> f64 { x.max(0.0).min(6.0) }

fn sample_f32(i: usize) -> f32 { ((i % 200) as f32) * 0.07 - 2.0 }
fn sample_f64(i: usize) -> f64 { ((i % 200) as f64) * 0.07 - 2.0 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "relu6 f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "relu6 f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "relu6 f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "relu6 bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("relu6 f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("relu6 f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("relu6 bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("relu6 f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn relu6_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
