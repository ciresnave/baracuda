//! Real-GPU smoke test for the gated ReGLU backward.
//!
//! Forward: `y = a · relu(b)`. Backward (saved x):
//!   da = (b > 0) ? dy·b : 0
//!   db = (b > 0) ? dy·a : 0
//! Bit-exact at f32 / f64; 1 ULP detour rounding at f16 / bf16.
...
fn cpu_ref_bw_f32(x: &[f32], dy: &[f32], input_shape: &[i32], split_dim: usize) -> Vec<f32> {
    ...
    for i in 0..out_numel {
        ...
        let (da, db) = if b > 0.0 { (dyv * b, dyv * a) } else { (0.0, 0.0) };
...
fn reglu_bw_f32() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::ReGlu, input_shape, split_dim, element: ElementKind::F32 };
    ...
    for i in 0..in_numel {
        assert_eq!(got[i].to_bits(), expected[i].to_bits(),
            "reglu bw f32 @ {i}: got {} exp {}", got[i], expected[i]);
    }
}
...
fn reglu_bw_f64() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::ReGlu, input_shape, split_dim, element: ElementKind::F64 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        assert_eq!(got[i].to_bits(), expected[i].to_bits(),
            "reglu bw f64 @ {i}: got {} exp {}", got[i], expected[i]);
    }
}
...
fn reglu_bw_f16() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::ReGlu, input_shape, split_dim, element: ElementKind::F16 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        let exp = expected[i];
        let g = got[i].to_f32();
        let allow = exp.abs().max(1.0) * F16_EPS;
        assert!((g - exp).abs() <= allow, "reglu bw f16 @ {i}: got {g} exp {exp}");
    }
}
...
fn reglu_bw_bf16() {
    ...
    let desc = GatedActivationBackwardDescriptor { kind: GatedActivationKind::ReGlu, input_shape, split_dim, element: ElementKind::Bf16 };
    let output_shape = desc.output_shape();
    let out_numel: usize = output_shape.iter().map(|&d| d as usize).product();
    ...
    for i in 0..in_numel {
        let exp = expected[i];
        let g = got[i].to_f32();
        let allow = exp.abs().max(1.0) * BF16_EPS;
        assert!((g - exp).abs() <= allow, "reglu bw bf16 @ {i}: got {g} exp {exp}");
    }
}
