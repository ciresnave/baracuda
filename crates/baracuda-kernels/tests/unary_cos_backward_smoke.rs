//! Real-GPU smoke test for unary Cos backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Cos`). Backward: `dx = -dy * sin(x)`.
//! Saved-x, transcendental.
//!
//! `sinf` is not bit-exact between Rust libm and CUDA libdevice, so all
//! dtypes use a tolerance compare.
...
#[test]
#[ignore]
fn cos_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f32> = (0..numel).map(|i| ((i % 100) as f32) * 0.02 - 1.0).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cos, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = -host_dy[i] * host_x[i].sin();
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "cos bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn cos_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.02 - 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cos, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cos bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn cos_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.02 - 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cos, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cos bw bf16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn cos_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| ((i % 100) as f64) * 0.02 - 1.0).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cos, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "cos bw f64 @ {i}");
    }
}
