...
fn clamp_backward_f16_3d() {
    ...
    let host_dy: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 47) as f32) * 0.25 - 5.0))
        .collect();
...
fn clamp_backward_bf16_3d() {
    ...
    let host_dy: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 47) as f32) * 0.25 - 5.0))
        .collect();
