...
#[test]
#[ignore]
fn segment_prod_f32_basic() {
    let (ctx, stream) = setup();
    let n: i32 = 6;
    let d: i32 = 2;
    let ns: i32 = 3;
    let seg: Vec<i32> = vec![0, 0, 0, 1, 2, 2];
    let input: Vec<f32> = vec![2.0, 3.0, 1.0, 4.0, 0.5, 2.0, 7.0, 5.0, 3.0, 1.5, 2.0, 4.0];
    ...
    let desc = SegmentProdDescriptor {
        num_inputs: n,
        embedding_dim: d,
        num_segments: ns,
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            (g - e).abs() <= tol,
            "segment_prod f32 mismatch @ {i}: got {g} expected {e} tol {tol}"
        );
    }
}
