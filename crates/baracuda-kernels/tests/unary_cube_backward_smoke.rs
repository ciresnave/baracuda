//! Real-GPU smoke test for unary cube backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Cube`).
//! Backward: `dx = dy * 3 * x²`. Saved-x, no transcendental.
...
#[test]
#[ignore]
fn cube_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    // Keep |x| modest so x² stays well within f16 range (~65k).
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cube, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] * 3.0 * host_x[i] * host_x[i];
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "cube bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn cube_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cube, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "cube bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn cube_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cube, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cube bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn cube_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cube, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cube bw bf16 @ {i}: got {g} exp {exp}");
    }
}
