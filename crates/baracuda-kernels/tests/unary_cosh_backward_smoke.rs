//! Real-GPU smoke test for unary Cosh backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Cosh`). Backward:
//! `dx = dy * sinh(x)`. Saved-x, transcendental.
...
fn cosh_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cosh, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] * host_x[i].sinh();
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "cosh bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn cosh_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cosh, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cosh bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn cosh_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cosh, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "cosh bw bf16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn cosh_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Cosh, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "cosh bw f64 @ {i}");
    }
}
