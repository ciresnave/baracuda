//! Real-GPU smoke test for the Phase 3 fanout binary-div kernel in
//! `baracuda-kernels-sys`.
//!
//! Covers `BinaryPlan<f32, N> + BinaryKind::Div` over a contiguous
...
//!
//! Inputs are constructed so that `b` never hits zero (offset by `+ 1.0`
//! instead of the trailblazer's `+ 3.75`); this avoids producing 0/0
//! NaNs whose payload bits aren't guaranteed identical between the
//! host f32 path and the device f32 path.
//!
...
fn run_case<const N: usize>(shape: [i32; N]) {
    ...
    // result equals the f32 div of the corresponding cells in A and
    // B exactly.
    ...
    if mismatches > 0 {
        ...
        panic!(
            "binary div f32: {mismatches} mismatches over {numel} cells \
...
// --- f16 / bf16 / f64 fanout ------------------------------------------------
//
// Inputs are bounded so quotients stay representable: `b` is bumped well
// above zero (>= ~1.0) so 1/b doesn't blow up and 0/0 NaNs can't appear.
...
fn run_case_f16_3d(shape: [i32; 3]) {
    ...
    let host_a: Vec<f16> = (0..numel)
        ...
        .collect();
    // b ranges in [1.0, ~10.25] — strictly positive, well clear of zero.
    let host_b: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 37) as f32) * 0.25 + 1.0))
        .collect();
...
fn run_case_bf16_3d(shape: [i32; 3]) {
    ...
    let host_b: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 37) as f32) * 0.25 + 1.0))
        .collect();
