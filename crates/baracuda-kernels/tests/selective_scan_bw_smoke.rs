...
use baracuda_kernels::{
    ElementKind, PlanPreference, SelectiveScanArgs, SelectiveScanBackwardArgs,
    SelectiveScanBackwardDescriptor, SelectiveScanBackwardPlan, SelectiveScanDescriptor,
    SelectiveScanPlan, TensorMut, TensorRef, Workspace, contiguous_stride,
};
...
fn run_fw_and_sum(
    ...
    let plan =
        SelectiveScanPlan::<f32>::select(stream, &desc, PlanPreference::default()).expect("select");
    ...
    plan.run(
        ...
        SelectiveScanArgs {
            u: TensorRef {
                data: u_dev.as_slice(),
                shape: s_ud,
                stride: contiguous_stride(s_ud),
            },
            delta: TensorRef {
                data: delta_dev.as_slice(),
                shape: s_ud,
                stride: contiguous_stride(s_ud),
            },
            a: TensorRef {
                data: a_dev.as_slice(),
                shape: s_a,
                stride: contiguous_stride(s_a),
            },
            b: TensorRef {
                data: b_dev.as_slice(),
                shape: s_bc,
                stride: contiguous_stride(s_bc),
            },
            c: TensorRef {
                data: c_dev.as_slice(),
                shape: s_bc,
...
fn selective_scan_bw_du_ddelta_da_match_fd_others_finite() {
    ...
    let u_host: Vec<f32> = (0..BSZ * L * DIM)
        .map(|i| 0.1 + (i as f32) * 0.01)
        .collect();
    let delta_host: Vec<f32> = (0..BSZ * L * DIM)
        .map(|i| 0.1 + (i as f32) * 0.005)
        .collect();
    let a_host: Vec<f32> = (0..DIM * DSTATE)
        .map(|i| -0.5 - (i as f32) * 0.02)
        .collect();
    let b_host: Vec<f32> = (0..BSZ * L * DSTATE)
        .map(|i| ((i as f32) * 0.07).sin() * 0.3)
        .collect();
    let c_host: Vec<f32> = (0..BSZ * L * DSTATE)
    ...
    let bw_plan =
        SelectiveScanBackwardPlan::<f32>::select(&stream, &bw_desc, PlanPreference::default())
            .expect("bw select");
    ...
    bw_plan
        .run(
            ...
            SelectiveScanBackwardArgs {
                ...
                delta_bias: None,
                dy: TensorRef {
                    data: dy_dev.as_slice(),
                    shape: s_ud,
                    stride: contiguous_stride(s_ud),
                },
                du: TensorMut {
                    data: du_dev.as_slice_mut(),
                    shape: s_ud,
                    stride: contiguous_stride(s_ud),
                },
                d_b: TensorMut {
                    data: db_dev.as_slice_mut(),
                    shape: s_bc,
                    stride: contiguous_stride(s_bc),
                },
                d_c: TensorMut {
                    data: dc_dev.as_slice_mut(),
                    shape: s_bc,
                    stride: contiguous_stride(s_bc),
                },
                d_delta: TensorMut {
                    data: ddelta_dev.as_slice_mut(),
                    shape: s_ud,
                    stride: contiguous_stride(s_ud),
                },
                d_a: TensorMut {
                    data: da_dev.as_slice_mut(),
                    shape: s_a,
                    stride: contiguous_stride(s_a),
                },
                d_d: None,
    ...
    for &i in &[0usize, BSZ * L * DIM - 1] {
        let mut up = u_host.clone();
        up[i] += eps;
        let mut um = u_host.clone();
        um[i] -= eps;
    ...
    {
        let i = 0usize;
        let mut dp = delta_host.clone();
        dp[i] += eps;
        let mut dm = delta_host.clone();
        dm[i] -= eps;
    ...
    {
        let i = 0usize;
        let mut ap = a_host.clone();
        ap[i] += eps;
        let mut am = a_host.clone();
        am[i] -= eps;
...
fn selective_scan_bw_with_d_skip_dz_ddelta_bias_runs_finite() {
    ...
    let z_host: Vec<f32> = (0..BSZ * L * DIM)
    ...
    bw_plan
        .run(
            ...
            SelectiveScanBackwardArgs {
                ...
                d_skip: Some(TensorRef {
                    data: d_dev.as_slice(),
                    shape: s_d,
                    stride: contiguous_stride(s_d),
                }),
                z: Some(TensorRef {
                    data: z_dev.as_slice(),
                    shape: s_ud,
                ...
                delta_bias: Some(TensorRef {
                    data: db_dev.as_slice(),
                    shape: s_d,
                    stride: contiguous_stride(s_d),
                }),
                dy: TensorRef {
                ...
                d_d: Some(TensorMut {
                    data: dD_dev.as_slice_mut(),
                    shape: s_d,
                    stride: contiguous_stride(s_d),
                }),
                dz: Some(TensorMut {
                    data: dz_dev.as_slice_mut(),
                    shape: s_ud,
                    stride: contiguous_stride(s_ud),
                }),
                d_delta_bias: Some(TensorMut {
                    data: ddb_dev.as_slice_mut(),
                    shape: s_d,
                    stride: contiguous_stride(s_d),
                }),
            },
    ...
    for v in &all {
        assert!(v.is_finite(), "du non-finite");
    }
    ddelta_dev.copy_to_host(&mut all).expect("dl");
    for v in &all {
        assert!(v.is_finite(), "ddelta non-finite");
    }
    dz_dev.copy_to_host(&mut all).expect("dl");
    for v in &all {
        assert!(v.is_finite(), "dz non-finite");
    }
    dD_dev.copy_to_host(&mut tmp_d).expect("dl");
    for v in &tmp_d {
        assert!(v.is_finite(), "dD non-finite");
    }
    ddb_dev.copy_to_host(&mut tmp_d).expect("dl");
    for v in &tmp_d {
        assert!(v.is_finite(), "d_delta_bias non-finite");
    }
}
