...
//! `UnaryPlan<T, N> + UnaryKind::Exp2` across f32 / f16 / bf16 / f64.
//!
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "exp2 f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "exp2 f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "exp2 f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "exp2 bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let expected: Vec<f32> = host_x.iter().map(|x| (*x).exp2()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("exp2 f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<f16> = host_x
        .iter()
        .map(|x| f16::from_f32({ let x = x.to_f32().exp2(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("exp2 f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<bf16> = host_x
        .iter()
        .map(|x| bf16::from_f32({ let x = x.to_f32().exp2(); x }))
        .collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("exp2 bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let expected: Vec<f64> = host_x.iter().map(|x| (*x).exp2()).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("exp2 f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn exp2_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            let x_lin = j * M + i;
            let y_lin = i * N_DIM + j;
            expected[y_lin] = { let x = x_buf[x_lin].exp2(); x };
        }
    }
