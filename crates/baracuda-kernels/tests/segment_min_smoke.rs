//! Real-GPU smoke test for `SegmentMinPlan<T>` (Phase 7 7.6).
//! `#[ignore]` by default.

use baracuda_driver::{init, Context, Device, DeviceBuffer, Stream};
use baracuda_kernels::{
    contiguous_stride, ElementKind, PlanPreference, SegmentMinArgs, SegmentMinDescriptor,
    SegmentMinPlan, TensorMut, TensorRef, Workspace,
};
...
#[test]
#[ignore]
fn segment_min_f32_basic() {
    let (ctx, stream) = setup();
    let n: i32 = 6;
    let d: i32 = 2;
    let ns: i32 = 3;
    let seg: Vec<i32> = vec![0, 0, 1, 1, 2, 2];
    let input: Vec<f32> = vec![3.0, 5.0, 1.0, 7.0, -1.0, 8.0, 2.0, 4.0, 9.0, -3.0, 6.0, 0.0];
    ...
    let plan =
        SegmentMinPlan::<f32>::select(&stream, &desc, PlanPreference::default()).expect("select");
    let args = SegmentMinArgs::<f32> {
        input: TensorRef {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "segment_min f32 mismatch @ {i}: got {g} expected {e}");
    }
}
