...
//! `UnaryPlan<T, N> + UnaryKind::Softsign` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - Pure arithmetic - no transcendentals - so close to bit-stable between the kernel's `fabsf` + division and Rust's `f32::abs` + division.
//!
//! Input range: linear `-17.25 + 0.5 * i`. Softsign is smooth and bounded in `(-1, 1)` so dynamic range is forgiving.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_softsign_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { { let f = x; f / (1.0 + f.abs()) } }
fn cpu_ref_f64(x: f64) -> f64 { { let f = x; f / (1.0 + f.abs()) } }

fn sample_f32(i: usize) -> f32 { (i as f32) * 0.5 - 17.25 }
fn sample_f64(i: usize) -> f64 { (i as f64) * 0.5 - 17.25 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softsign f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softsign f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softsign f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softsign bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softsign f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softsign f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softsign bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softsign f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn softsign_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
