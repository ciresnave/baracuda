...
fn host_cumprod_bw_f32<const N: usize>(
    ...
    for_each_coord::<N, _>(shape, |coord, linear| {
        ...
        for d in 0..N {
            idx_j += coord[d] as usize * stride[d];
        }
        let x_j = x[idx_j];
...
fn cumprod_bw_f64_2d_reverse_fw() {
    ...
    let expected_f32 =
        host_cumprod_bw_f32::<2>(shape, 0, true, &host_dy_f32, &host_x_f32, &host_y_f32);
...
fn cumprod_bw_f16_2d() {
    ...
    let expected_f32 =
        host_cumprod_bw_f32::<2>(shape, 1, false, &host_dy_f32, &host_x_f32, &host_y_f32);
    ...
    for i in 0..numel {
        ...
        assert!(
            diff <= tol,
            "f16 cumprod BW @ {i}: got={} want={} diff={}",
            got[i].to_f32(),
            expected_f32[i],
            diff
        );
