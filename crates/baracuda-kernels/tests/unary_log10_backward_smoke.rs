//! Real-GPU smoke test for unary log10 backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Log10`).
//! Backward: `dx = dy / (x * ln(10))`. Saved-x; the kernel hardcodes
//! `ln(10)`.
...
const LN10_F32: f32 = std::f32::consts::LN_10;
const LN10_F64: f64 = std::f64::consts::LN_10;
...
#[test]
#[ignore]
fn log10_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f32> = (0..numel).map(|i| (i as f32) * 0.0625 + 1.0).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log10, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] / (host_x[i] * LN10_F32);
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "log10 bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn log10_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.0625 + 1.0).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log10, shape, element: ElementKind::F64 };
    ...
    dev_dx.copy_to_host(&mut got).expect("download");
    for i in 0..numel {
        let exp = host_dy[i] / (host_x[i] * LN10_F64);
        let tol = exp.abs().max(1.0) * 4.0 * f64::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "log10 bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn log10_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.0625 + 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log10, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp = dy / (x * LN10_F32);
        let g = got[i].to_f32();
        let tol = exp.abs().max(1.0) * 4.0 * F16_EPS;
        assert!((g - exp).abs() <= tol, "log10 bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn log10_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.0625 + 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Log10, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "log10 bw bf16 @ {i}: got {g} exp {exp}");
    }
}
