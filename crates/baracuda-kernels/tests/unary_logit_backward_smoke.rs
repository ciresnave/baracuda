//! Real-GPU smoke test for unary Logit backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Logit`).
//! Backward: `dx = dy / (x * (1 - x))`. Saved-x, no transcendental.
//! Domain: strictly `0 < x < 1` — test data lives in `[0.1, 0.9]`.
...
#[test]
#[ignore]
fn logit_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f32> = (0..numel).map(|i| ((i % 81) as f32) * 0.01 + 0.1).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Logit, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] / (host_x[i] * (1.0 - host_x[i]));
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "logit bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn logit_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| ((i % 81) as f64) * 0.01 + 0.1).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Logit, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "logit bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn logit_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 81) as f32) * 0.01 + 0.1)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Logit, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "logit bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn logit_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 81) as f32) * 0.01 + 0.1)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Logit, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "logit bw bf16 @ {i}: got {g} exp {exp}");
    }
}
