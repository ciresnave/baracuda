...
use baracuda_kernels::{
    ...
    SelectiveScanArgs, SelectiveScanDescriptor, SelectiveScanPlan, TensorMut, TensorRef, Workspace,
    contiguous_stride,
};
...
fn mamba1_block_tiny_end_to_end() {
    ...
    conv_plan
        .run(
            ...
            CausalConv1dArgs {
                x: TensorRef {
                    data: x_conv_dev.as_slice(),
                    shape: s_x_conv,
                    stride: contiguous_stride(s_x_conv),
                },
                weight: TensorRef {
                    data: w_conv_dev.as_slice(),
                    shape: s_w_conv,
                    stride: contiguous_stride(s_w_conv),
                },
                bias: Some(TensorRef {
                    data: b_conv_dev.as_slice(),
                    shape: s_b_conv,
                    stride: contiguous_stride(s_b_conv),
                }),
                y: TensorMut {
                    data: u_dev.as_slice_mut(),
                    shape: s_x_conv,
                    stride: contiguous_stride(s_x_conv),
                },
            },
    ...
    assert!(
        max_abs < 100.0,
        "block output magnitude too large: {}",
        max_abs
    );
    let mean_abs: f32 = out.iter().map(|v| v.abs()).sum::<f32>() / (out.len() as f32);
    assert!(
        mean_abs > 1e-4,
        "block output mean magnitude suspiciously small: {}",
        mean_abs
    );
}
