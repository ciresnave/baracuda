...
use baracuda_kernels::{
    ElementKind, FractionalMaxPool2dBwArgs, FractionalMaxPool2dDescriptor,
    ...
    RandomDescriptor, RandomKind, RandomPlan, TensorMut, TensorRef, Workspace, contiguous_stride,
};
...
fn host_fmp2d_f32(
    ...
    for ni in 0..n {
        for ci in 0..c {
            ...
            for oh in 0..h_out {
                let start_h =
                    host_window_start(oh as i32, h_out as i32, h_in as i32, kh as i32, alpha_h)
                        as usize;
                for ow in 0..w_out {
                    let start_w =
                        host_window_start(ow as i32, w_out as i32, w_in as i32, kw as i32, alpha_w)
                            as usize;
                    let mut best_v = f32::NEG_INFINITY;
                    let mut best_idx = ((ni * c + ci) * h_in + start_h) * w_in + start_w;
                    for dh in 0..kh {
                        for dw in 0..kw {
                            let idx = ((ni * c + ci) * h_in + start_h + dh) * w_in + start_w + dw;
                            let v = x[idx];
                            if v > best_v {
                                best_v = v;
                                best_idx = idx;
...
fn host_fmp3d_f32(
    ...
    for ni in 0..n {
        for ci in 0..c {
            ...
            for od in 0..d_out {
                let start_d =
                    host_window_start(od as i32, d_out as i32, d_in as i32, kd as i32, alpha_d)
                        as usize;
                for oh in 0..h_out {
                    let start_h =
                        host_window_start(oh as i32, h_out as i32, h_in as i32, kh as i32, alpha_h)
...
fn fmp2d_f32_4x4_to_3x3() {
    ...
    plan.run_fw(
        ...
        FractionalMaxPool2dFwArgs {
            ...
            indices: TensorMut {
                data: dev_indices.as_slice_mut(),
                shape: y_shape,
                stride: contiguous_stride(y_shape),
            },
            random_samples: TensorRef {
                data: dev_samples.as_slice(),
                shape: s_shape,
    ...
    for i in 0..numel_y {
        ...
        assert!(
            ...
            "fmp2d f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
        assert_eq!(
            got_idx[i], exp_idx[i],
            "fmp2d f32 idx @ {i}: got={} want={}",
            got_idx[i], exp_idx[i]
        );
    }
...
fn fmp2d_f32_8x8_to_5x5_random_samples() {
    ...
    for i in 0..numel_y {
        let idx = got_idx[i];
        assert!(
            idx >= 0 && (idx as usize) < numel_x,
            "fmp2d rand idx @ {i}: {idx} out of range"
        );
        assert!(
            got_y[i].is_finite() && got_y[i] >= x_min && got_y[i] <= x_max,
            "fmp2d rand y @ {i}: {} out of input range [{}, {}]",
            got_y[i],
            x_min,
            x_max
        );
        ...
        assert!(
            (got_y[i] - expected).abs() <= 1e-6,
            "fmp2d rand y @ {i}: got={} expected x[{}]={}",
            got_y[i],
            idx,
            expected
        );
...
fn fmp3d_f32_4x4x4_to_3x3x3() {
    ...
    for i in 0..numel_y {
        ...
        assert!(
            (got_y[i] - exp_y[i]).abs() <= tol,
            "fmp3d f32 y @ {i}: got={} want={}",
            got_y[i],
            exp_y[i]
        );
        assert_eq!(
            got_idx[i], exp_idx[i],
            "fmp3d f32 idx @ {i}: got={} want={}",
            got_idx[i], exp_idx[i]
        );
    }
}
