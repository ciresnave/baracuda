...
use baracuda_kernels::{
    ...
    SsdChunkScanArgs, SsdChunkScanDescriptor, SsdChunkScanPlan, TensorMut, TensorRef, Workspace,
    contiguous_stride,
};
...
fn mamba2_block_tiny_end_to_end() {
        ...
        .run(
            ...
            CausalConv1dArgs {
                x: TensorRef {
                    ...
                    shape: shape_xconv,
                    stride: contiguous_stride(shape_xconv),
                },
                weight: TensorRef {
                    data: w_conv_dev.as_slice(),
                    shape: shape_wconv,
                    stride: contiguous_stride(shape_wconv),
                },
                bias: Some(TensorRef {
                    data: b_conv_dev.as_slice(),
                    shape: shape_bconv,
                    stride: contiguous_stride(shape_bconv),
                }),
                y: TensorMut {
    ...
    ssd_plan
        .run(
            ...
            SsdChunkScanArgs {
                x: TensorRef {
                    data: u_ssd_dev.as_slice(),
                    shape: shape_x,
                    stride: contiguous_stride(shape_x),
                },
                dt: TensorRef {
                    data: dt_dev.as_slice(),
                    shape: shape_dt,
                    stride: contiguous_stride(shape_dt),
                },
                a: TensorRef {
                    data: a_dev.as_slice(),
                    shape: shape_a,
                    stride: contiguous_stride(shape_a),
                },
                b: TensorRef {
                    data: b_proj_dev.as_slice(),
                    shape: shape_bn,
                    stride: contiguous_stride(shape_bn),
                },
                c: TensorRef {
                    data: c_proj_dev.as_slice(),
                    shape: shape_bn,
                    stride: contiguous_stride(shape_bn),
                },
                y: TensorMut {
