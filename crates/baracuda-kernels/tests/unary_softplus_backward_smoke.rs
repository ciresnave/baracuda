//! Real-GPU smoke test for unary Softplus backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Softplus`).
//!
//! Forward: `y = log(1 + exp(x))`. Backward: `dx = dy / (1 + exp(-x))`.
//! Saved-x; smooth. 4×eps tolerance — single `exp` chain.
...
#[test]
#[ignore]
fn softplus_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    // Span (-3, +3) to avoid f16 overflow of intermediate `exp(-x)` for large negative x.
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Softplus, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp_val = host_dy[i] / (1.0 + (-host_x[i]).exp());
        let tol = exp_val.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp_val).abs() <= tol, "softplus bw f32 @ {i}: got {} exp {}", got[i], exp_val);
    }
}

#[test]
#[ignore]
fn softplus_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Softplus, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        let exp_val = host_dy[i] / (1.0 + (-host_x[i]).exp());
        let tol = exp_val.abs().max(1.0) * 4.0 * f64::EPSILON;
        assert!((got[i] - exp_val).abs() <= tol, "softplus bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn softplus_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Softplus, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp_val = dy / (1.0 + (-fx).exp());
        let g = got[i].to_f32();
        let tol = exp_val.abs().max(1.0) * 4.0 * F16_EPS;
        assert!((g - exp_val).abs() <= tol, "softplus bw f16 @ {i}: got {g} exp {exp_val}");
    }
}

#[test]
#[ignore]
fn softplus_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Softplus, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        let exp_val = dy / (1.0 + (-fx).exp());
        let g = got[i].to_f32();
        let tol = exp_val.abs().max(1.0) * 4.0 * BF16_EPS;
        assert!((g - exp_val).abs() <= tol, "softplus bw bf16 @ {i}: got {g} exp {exp_val}");
    }
}
