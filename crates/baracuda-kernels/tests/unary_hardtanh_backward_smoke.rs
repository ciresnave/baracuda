//! Real-GPU smoke test for unary Hardtanh backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Hardtanh`).
//!
//! Forward: `y = clamp(x, -1, 1)`. Backward: `dx = (-1 < x < 1) ? dy : 0`.
//! Saved-x; piecewise activation BW.
//!
//! Bit-exact compare for every dtype — the kernel does one compare and
//! one bit-preserving select.
...
#[test]
#[ignore]
fn hardtanh_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f32> = (0..numel).map(|i| (i as f32) * 0.01 - 2.0).collect();
    ...
    for i in 0..numel {
        let exp = if host_x[i] > -1.0 && host_x[i] < 1.0 {
            host_dy[i]
        } else {
            0.0
        };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardtanh bw f32 @ {i}");
    }
}

#[test]
#[ignore]
fn hardtanh_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.01 - 2.0).collect();
    ...
    for i in 0..numel {
        ...
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardtanh bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn hardtanh_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32((i as f32) * 0.01 - 2.0))
        .collect();
    ...
    for i in 0..numel {
        ...
        let exp = if fx > -1.0 && fx < 1.0 {
            host_dy[i]
        } else {
            f16::from_f32(0.0)
        };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardtanh bw f16 @ {i}");
    }
}

#[test]
#[ignore]
fn hardtanh_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32((i as f32) * 0.01 - 2.0))
        .collect();
    ...
    for i in 0..numel {
        ...
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardtanh bw bf16 @ {i}");
    }
}
