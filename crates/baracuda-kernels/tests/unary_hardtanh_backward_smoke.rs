...
fn hardtanh_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardtanh, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = if host_x[i] > -1.0 && host_x[i] < 1.0 { host_dy[i] } else { 0.0 };
...
fn hardtanh_backward_f64_3d() {
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardtanh, shape, element: ElementKind::F64 };
...
fn hardtanh_backward_f16_3d() {
    ...
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32((i as f32) * 0.01 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardtanh, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp = if fx > -1.0 && fx < 1.0 { host_dy[i] } else { f16::from_f32(0.0) };
...
fn hardtanh_backward_bf16_3d() {
    ...
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32((i as f32) * 0.01 - 2.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardtanh, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        let fx = host_x[i].to_f32();
        let exp = if fx > -1.0 && fx < 1.0 { host_dy[i] } else { bf16::from_f32(0.0) };
