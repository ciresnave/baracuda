...
//! `UnaryPlan<T, N> + UnaryKind::Erf` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon — `erff` / `erf` libdevice routines may
//! diverge from Rust's `libm` by ~1 ULP.
//!
//! Input range: `[-5, 5]`. `erf` saturates near `±1` at `|x| ≳ 4`, so
//! the dynamic range is comfortable for half-precision.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_erf_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { libm::erff(x) }
fn cpu_ref_f64(x: f64) -> f64 { libm::erf(x) }

// Input range: [-5, 5] — `erf` saturates near ±1 outside this band.
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erf f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erf f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erf f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erf bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erf f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erf f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erf bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<f64> = (0..numel).map(|i| ((i % 200) as f64) * 0.05 - 5.0).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("erf f64 run");
    stream.synchronize().expect("sync");
