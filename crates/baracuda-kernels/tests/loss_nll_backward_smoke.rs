...
use baracuda_kernels::{
    ElementKind, LossReduction, NllLossBackwardArgs, NllLossBackwardDescriptor,
    NllLossBackwardPlan, PlanPreference, TensorMut, TensorRef, Workspace, contiguous_stride,
};
use half::{bf16, f16};
...
fn host_nll_bw_f64(n_rows: usize, classes: usize, target: &[i64], dy: f64) -> Vec<f64> {
...
fn loss_nll_backward_f32_mean() {
    ...
    let dy_host = [1.0f32];
    let expected: Vec<f32> = host_nll_bw_f64(n_rows as usize, class_extent as usize, &host_t, 1.0)
        .into_iter()
        .map(|v| v as f32)
        .collect();
...
fn loss_nll_backward_f64_mean() {
    ...
    let dy_host = [1.0f64];
    let expected = host_nll_bw_f64(n_rows as usize, class_extent as usize, &host_t, 1.0);
