...
fn hardshrink_backward_f32_3d() {
    ...
    for i in 0..numel {
        ...
        assert_eq!(
            got[i].to_bits(),
            exp.to_bits(),
            "hardshrink bw f32 @ {i}: x={} dy={} got {} exp {}",
            host_x[i],
            host_dy[i],
            got[i],
            exp
...
fn hardshrink_backward_f16_3d() {
    ...
    let host_x: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 401) as f32) * 0.01 - 2.0))
        .collect();
    ...
    dev_dx.copy_to_host(&mut got).expect("download");
    for i in 0..numel {
        let exp = if host_x[i].to_f32().abs() > 0.5 {
            host_dy[i]
        } else {
            f16::from_f32(0.0)
        };
...
fn hardshrink_backward_bf16_3d() {
    ...
    let host_x: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 401) as f32) * 0.01 - 2.0))
        .collect();
