//! Real-GPU smoke test for `UnaryBackwardPlan + UnaryKind::Hardshrink`.
//!
//! Forward: `y = x if |x| > λ else 0`. Backward: `dx = dy if |x| > λ
//! else 0`. λ hardcoded to 0.5.
//!
...
fn hardshrink_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardshrink, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        ...
        assert_eq!(got[i].to_bits(), exp.to_bits(),
            "hardshrink bw f32 @ {i}: x={} dy={} got {} exp {}", host_x[i], host_dy[i], got[i], exp);
    }
}

#[test]
#[ignore]
fn hardshrink_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardshrink, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        let exp = if host_x[i].abs() > 0.5 { host_dy[i] } else { 0.0 };
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardshrink bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn hardshrink_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardshrink, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardshrink bw f16 @ {i}");
    }
}

#[test]
#[ignore]
fn hardshrink_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Hardshrink, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert_eq!(got[i].to_bits(), exp.to_bits(), "hardshrink bw bf16 @ {i}");
    }
}
