...
fn expm1_backward_f32_3d() {
    ...
    let host_y: Vec<f32> = (0..numel)
        .map(|i| ((i % 100) as f32) * 0.02 - 0.5)
        .collect();
...
fn expm1_backward_f64_3d() {
    ...
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f64> = (0..numel)
        .map(|i| ((i % 100) as f64) * 0.02 - 0.5)
        .collect();
...
fn expm1_backward_f16_3d() {
    ...
    let host_y: Vec<f16> = (0..numel)
        .map(|i| f16::from_f32(((i % 100) as f32) * 0.02 - 0.5))
        .collect();
...
fn expm1_backward_bf16_3d() {
    ...
    let host_y: Vec<bf16> = (0..numel)
        .map(|i| bf16::from_f32(((i % 100) as f32) * 0.02 - 0.5))
        .collect();
