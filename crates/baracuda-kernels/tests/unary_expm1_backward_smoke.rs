//! Real-GPU smoke test for unary expm1 backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Expm1`). Backward:
//! `dx = dy * (y + 1)`. Saved-y.
...
#[test]
#[ignore]
fn expm1_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f32> = (0..numel).map(|i| ((i % 100) as f32) * 0.02 - 0.5).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Expm1, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] * (host_y[i] + 1.0);
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "expm1 bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn expm1_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f64> = (0..numel).map(|i| ((i % 100) as f64) * 0.02 - 0.5).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Expm1, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "expm1 bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn expm1_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.02 - 0.5)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Expm1, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "expm1 bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn expm1_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.02 - 0.5)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Expm1, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "expm1 bw bf16 @ {i}: got {g} exp {exp}");
    }
}
