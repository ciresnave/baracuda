...
fn gelu_f32(b: f32) -> f32 {
    0.5 * b * (1.0 + erf_f32(b * INV_SQRT2_F))
}
fn gelu_f64(b: f64) -> f64 {
    0.5 * b * (1.0 + erf_f64(b * INV_SQRT2_D))
}
...
fn geglu_fw_f32() {
    ...
    let desc = GatedActivationDescriptor {
        kind: GatedActivationKind::GeGlu,
        input_shape,
        split_dim,
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            (g - e).abs() <= allow,
            "geglu f32 @ {i}: got {g} exp {e} allow {allow}"
        );
    }
}
...
fn geglu_fw_f64() {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            (g - e).abs() <= allow,
            "geglu f64 @ {i}: got {g} exp {e} allow {allow}"
        );
    }
}
