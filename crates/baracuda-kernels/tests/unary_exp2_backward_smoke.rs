//! Real-GPU smoke test for unary exp2 backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Exp2`).
//! Backward: `dx = dy * y * ln(2)`. Saved-y; the kernel hardcodes
//! `ln(2)`.
...
fn exp2_backward_f32_3d() {
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Exp2, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] * host_y[i] * LN2_F32;
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "exp2 bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn exp2_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Exp2, shape, element: ElementKind::F64 };
    ...
    dev_dx.copy_to_host(&mut got).expect("download");
    for i in 0..numel {
        let exp = host_dy[i] * host_y[i] * LN2_F64;
        let tol = exp.abs().max(1.0) * 4.0 * f64::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "exp2 bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn exp2_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Exp2, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        let exp = dy * y * LN2_F32;
        let g = got[i].to_f32();
        let tol = exp.abs().max(1.0) * 4.0 * F16_EPS;
        assert!((g - exp).abs() <= tol, "exp2 bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn exp2_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Exp2, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "exp2 bw bf16 @ {i}: got {g} exp {exp}");
    }
}
