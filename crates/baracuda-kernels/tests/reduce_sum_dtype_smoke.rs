//! Real-GPU dtype-fanout smoke tests for `ReducePlan<T, 3> +
//! ReduceKind::Sum` across `{f16, bf16, f64}`. The `f32` cell is covered
//! by the trailblazer file `reduce_sum_smoke.rs`.
//!
//! Shape: `[4, 16, 32]` with axis-1 reduction (matches one of the
//! trailblazer cases). Host reference does sequential in-order
//! accumulation to mirror the GPU's per-output-cell loop.
//!
//! Tolerance per dtype:
//! - `f64`: bit-exact (sequential f64 accumulation matches host).
//! - `f16` / `bf16`: relative `4 * eps` since every step detours
//!   through f32 (round → f32 → add → round → f16/bf16).
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \
//!   --test reduce_sum_dtype_smoke -- --ignored`.
...
// Per-step ULP tolerances mirroring the binary dtype fanout file.
...
#[test]
#[ignore]
fn reduce_sum_dtype_f16() {
    let (ctx, stream) = setup();

    // Small magnitudes — Sum over 16 cells must stay well under f16 max.
    let host_x: Vec<f16> = (0..in_numel())
        .map(|i| f16::from_f32(((i % 41) as f32) * 0.125 - 2.0))
        .collect();

    // Host reference: in-order sum mirroring the GPU's k=0..16 loop.
    ...
    for i in 0..out_numel() {
        ...
        for k in 0..SHAPE[AXIS] {
            ...
            // f32-detour to match the kernel.
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            diff <= allow,
            "reduce sum f16 mismatch @ {i}: got {gf} expected {ef}, diff {diff} allow {allow}"
        );
    }
}
...
fn reduce_sum_dtype_bf16() {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            diff <= allow,
            "reduce sum bf16 mismatch @ {i}: got {gf} expected {ef}, diff {diff} allow {allow}"
        );
    }
}
...
fn reduce_sum_dtype_f64() {
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(
            g.to_bits(),
            e.to_bits(),
            "reduce sum f64 mismatch @ {i}: got {g} expected {e}"
        );
    }
}
