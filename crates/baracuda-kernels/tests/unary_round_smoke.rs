...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    let host_expected: Vec<f16> = host_x
        ...
        .map(|x| {
            f16::from_f32({
                let x = x.to_f32().round_ties_even();
                x
            })
        })
        .collect();
...
fn round_f32_strided_transposed() {
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            ...
            let y_lin = i * N_DIM + j;
            expected[y_lin] = {
                let x = x_buf[x_lin].round_ties_even();
                x
            };
        }
    }
