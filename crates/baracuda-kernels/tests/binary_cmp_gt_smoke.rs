//! Real-GPU smoke test for `BinaryCmpPlan<T, N> + BinaryCmpKind::Gt`
//! across the 4 FP dtypes (f32 / f16 / bf16 / f64).
//!
//! IEEE semantics: any comparison with NaN returns 0. Inputs are kept
//! within `[-10, 10]` (f16/bf16 representable) with a mix of `a > b`,
//! `a < b`, and `a == b` cells so the kernel exercises all three
//! ordering outcomes.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \
//!   --test binary_cmp_gt_smoke -- --ignored`.
...
fn cmp_gt_f32_3d() {
    ...
    let host_a: Vec<f32> = (0..numel)
        ...
        .collect();
    // Mix three regimes via mod 3:
    //   i%3==0 → b == a   (a > b → false)
    //   i%3==1 → b smaller (a > b → true)
    //   i%3==2 → b larger  (a > b → false)
    ...
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp gt f32 mismatch @ {i}");
    }
...
fn cmp_gt_f16_3d() {
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp gt f16 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
fn cmp_gt_bf16_3d() {
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp gt bf16 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
fn cmp_gt_f64_3d() {
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp gt f64 mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
...
#[test]
#[ignore]
fn cmp_gt_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    let a_buf: Vec<f32> = (0..(N_DIM * M))
        .map(|i| ((i % 41) as f32) * 0.5 - 10.0)
        .collect();
    // For each (i, j), a_logical = a_buf[j*M + i].
    ...
    dev_y.copy_to_host(&mut got).expect("download");
    for (i, (g, e)) in got.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "cmp gt f32 strided mismatch @ {i}");
    }
    let zeros = got.iter().filter(|&&x| x == 0).count();
    let ones = got.iter().filter(|&&x| x == 1).count();
    assert!(zeros > 0 && ones > 0, "expected mixed, got zeros={zeros} ones={ones}");
