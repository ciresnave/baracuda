...
use baracuda_kernels::{
    ElementKind, PlanPreference, SelectiveScanArgs, SelectiveScanDescriptor, SelectiveScanPlan,
...
fn check_close(a: &[f32], b: &[f32], tol: f32, tag: &str) {
    ...
    for (i, (av, bv)) in a.iter().zip(b.iter()).enumerate() {
        ...
        if diff > tol * scale {
            panic!(
                "{}: mismatch at idx {} — got {}, expected {}, diff {}",
                tag, i, av, bv, diff
            );
        }
    }
...
fn selective_scan_f32_vanilla_matches_cpu_ref() {
    ...
    let got = run_fw_f32(
        &ctx, &stream, &u, &delta, &a, &b, &c, None, None, None, false,
    );
    let expected = cpu_ref_selective_scan_f32(
        &u, &delta, &a, &b, &c, None, None, None, false, BSZ, L, DIM, DSTATE,
    );
...
fn selective_scan_f32_with_delta_bias_and_softplus() {
    ...
    check_close(
        ...
        &expected,
        1e-4,
        "selective_scan_f32_with_delta_bias_and_softplus",
    );
}
...
fn selective_scan_f16_vanilla_matches_cpu_ref_loose() {
    ...
    let expected = cpu_ref_selective_scan_f32(
        &u_f, &delta_f, &a_f, &b_f, &c_f, None, None, None, false, BSZ, L, DIM, DSTATE,
    );
...
fn selective_scan_with_last_state_saves_final_h() {
    ...
    plan.run(
        ...
        SelectiveScanArgs {
            ...
            last_state: Some(TensorMut {
                data: ls_dev.as_slice_mut(),
                shape: s_ls,
                stride: contiguous_stride(s_ls),
            }),
        },
    )
    .expect("run");
