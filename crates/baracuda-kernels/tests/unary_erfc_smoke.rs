...
//! `UnaryPlan<T, N> + UnaryKind::Erfc` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - `erfcf` / `erfc` libdevice routines may diverge from Rust's `libm` by ~1 ULP.
//!
//! Input range: `[-5, 5]`. `erfc` saturates near `0` for large positive x and near `2` for large negative x; the `max(|expected|, 1.0)` floor in the tolerance helper absorbs the small-value end.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_erfc_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { libm::erfcf(x) }
fn cpu_ref_f64(x: f64) -> f64 { libm::erfc(x) }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erfc f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erfc f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erfc f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "erfc bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erfc f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erfc f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erfc bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("erfc f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn erfc_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
