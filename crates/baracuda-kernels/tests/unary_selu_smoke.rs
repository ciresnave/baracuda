...
fn cpu_ref_f32(x: f32) -> f32 {
    {
        const A: f32 = 1.6732632423543772;
        const L: f32 = 1.0507009873554805;
        let branch = if x > 0.0 { x } else { A * (x.exp() - 1.0) };
        L * branch
    }
}
fn cpu_ref_f64(x: f64) -> f64 {
    {
        ...
        const L: f64 = 1.0507009873554805;
        let branch = if x > 0.0 { x } else { A * (x.exp() - 1.0) };
        L * branch
    }
}
...
fn sample_f32(i: usize) -> f32 {
    ((i % 200) as f32) * 0.05 - 5.0
}
fn sample_f64(i: usize) -> f64 {
    ((i % 200) as f64) * 0.05 - 5.0
}
