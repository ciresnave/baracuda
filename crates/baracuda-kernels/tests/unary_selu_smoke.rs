...
//! `UnaryPlan<T, N> + UnaryKind::Selu` across f32 / f16 / bf16 / f64.
//!
//! Four contig tests (one per dtype, shape `[8, 128, 128]`) + one
//! f32 strided test. Uses a relative-tolerance compare with a 4x fudge
//! over the dtype epsilon - `expf` / `exp` libdevice routines may diverge from Rust's `f32::exp` / `f64::exp` by ~1 ULP; the `lambda * (alpha * (exp - 1))` chain rounds together with the host reference.
//!
//! Input range: `[-5, 5]`. Exercises both branches; for `x <= ~-5`, `exp(x)` is small so the negative side saturates near `-lambda * alpha`.
//!
//! `#[ignore]` by default; run with
//! `cargo test -p baracuda-kernels --release --features sm89 \`
//!   `--test unary_selu_smoke -- --ignored`.
...
// 1-ULP relative-tolerance constants (same scheme as transcendental smoke tests).
...
fn cpu_ref_f32(x: f32) -> f32 { { const A: f32 = 1.6732632423543772; const L: f32 = 1.0507009873554805; let branch = if x > 0.0 { x } else { A * (x.exp() - 1.0) }; L * branch } }
fn cpu_ref_f64(x: f64) -> f64 { { const A: f64 = 1.6732632423543772; const L: f64 = 1.0507009873554805; let branch = if x > 0.0 { x } else { A * (x.exp() - 1.0) }; L * branch } }

fn sample_f32(i: usize) -> f32 { ((i % 200) as f32) * 0.05 - 5.0 }
fn sample_f64(i: usize) -> f64 { ((i % 200) as f64) * 0.05 - 5.0 }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "selu f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "selu f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "selu f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "selu bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    let host_x: Vec<f32> = (0..numel).map(sample_f32).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("selu f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<f16> = (0..numel).map(|i| f16::from_f32(sample_f32(i))).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("selu f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(sample_f32(i))).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("selu bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    let host_x: Vec<f64> = (0..numel).map(sample_f64).collect();
    ...
    plan.run(&stream, Workspace::None, args).expect("selu f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn selu_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
    ...
    let x_buf: Vec<f32> = (0..(N_DIM * M)).map(sample_f32).collect();
    let x_shape = [m, n];
    ...
    for i in 0..M {
        for j in 0..N_DIM {
            let x_lin = j * M + i;
            let y_lin = i * N_DIM + j;
            expected[y_lin] = cpu_ref_f32(x_buf[x_lin]);
        }
    }
