//! Real-GPU smoke test for `BinaryPlan + BinaryKind::Fmax` —
//! IEEE 754 `fmax(a, b)`. NaN-aware: returns the non-NaN operand
...
fn fmax_ref_f32(a: f32, b: f32) -> f32 {
...
fn fmax_ref_f64(a: f64, b: f64) -> f64 {
    if a.is_nan() && b.is_nan() {
        a
    } else if a.is_nan() {
        b
