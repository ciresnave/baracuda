...
use baracuda_kernels::{
    ...
    CausalConv1dBackwardPlan, CausalConv1dDescriptor, CausalConv1dPlan, ElementKind,
...
fn causal_conv1d_bw_dx_dw_db_match_fd() {
    ...
    let bw_plan =
        CausalConv1dBackwardPlan::<f32>::select(&stream, &bw_desc, PlanPreference::default())
            .expect("bw select");
    ...
    for &i in &[0usize, C * W / 2, C * W - 1] {
        let mut wp = weight_host.clone();
        wp[i] += eps;
        let mut wm = weight_host.clone();
        wm[i] -= eps;
    ...
    for i in 0..C {
        let mut bp = bias_host.clone();
        bp[i] += eps;
        let mut bm = bias_host.clone();
        bm[i] -= eps;
