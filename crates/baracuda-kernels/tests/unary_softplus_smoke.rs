...
//! `UnaryPlan<T, N> + UnaryKind::Softplus` across f32 / f16 / bf16 / f64.
//!
...
//!   `--test unary_softplus_smoke -- --ignored`.
...
fn cpu_ref_f32(x: f32) -> f32 { { if x > 20.0 { x } else { (1.0 + x.exp()).ln() } } }
fn cpu_ref_f64(x: f64) -> f64 { { if x > 20.0 { x } else { (1.0 + x.exp()).ln() } } }
...
fn assert_close_f32(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softplus f32 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f64(got: f64, expected: f64, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softplus f64 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_f16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softplus f16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn assert_close_bf16(got: f32, expected: f32, idx: usize) {
    ...
    assert!(
        diff <= allow,
        "softplus bf16 mismatch @ {idx}: got {got} expected {expected} (diff {diff} > allow {allow})"
    );
}
...
fn run_f32_contig<const N: usize>(shape: [i32; N]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softplus f32 run");
...
fn run_f16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softplus f16 run");
    stream.synchronize().expect("sync");
...
fn run_bf16_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softplus bf16 run");
    stream.synchronize().expect("sync");
...
fn run_f64_contig_3d(shape: [i32; 3]) {
    ...
    plan.run(&stream, Workspace::None, args).expect("softplus f64 run");
    stream.synchronize().expect("sync");
...
#[test]
#[ignore]
fn softplus_f32_strided_transposed() {
    let (ctx, stream) = setup();
    const M: usize = 48;
