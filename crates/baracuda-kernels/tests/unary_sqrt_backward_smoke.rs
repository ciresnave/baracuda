//! Real-GPU smoke test for unary sqrt backward
//! (`UnaryBackwardPlan<T, 3> + UnaryKind::Sqrt`). Backward:
//! `dx = dy / (2 * y)`. Saved-y. `y` is bounded well above zero
//! (>= 1.0) so 1/y stays finite and bit patterns stay defined.
...
#[test]
#[ignore]
fn sqrt_backward_f32_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f32> = (0..numel).map(|i| (i as f32) * 0.0625 + 1.0).collect();
    ...
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Sqrt, shape, element: ElementKind::F32 };
    ...
    for i in 0..numel {
        let exp = host_dy[i] / (2.0 * host_y[i]);
        let tol = exp.abs().max(1.0) * 4.0 * f32::EPSILON;
        assert!((got[i] - exp).abs() <= tol, "sqrt bw f32 @ {i}: got {} exp {}", got[i], exp);
    }
}

#[test]
#[ignore]
fn sqrt_backward_f64_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f64> = (0..numel).map(|i| (i as f64) * 0.0625 + 1.0).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Sqrt, shape, element: ElementKind::F64 };
    ...
    for i in 0..numel {
        ...
        assert!((got[i] - exp).abs() <= tol, "sqrt bw f64 @ {i}");
    }
}

#[test]
#[ignore]
fn sqrt_backward_f16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<f16> = (0..numel).map(|i| f16::from_f32(((i % 100) as f32) * 0.0625 + 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Sqrt, shape, element: ElementKind::F16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "sqrt bw f16 @ {i}: got {g} exp {exp}");
    }
}

#[test]
#[ignore]
fn sqrt_backward_bf16_3d() {
    let (ctx, stream) = setup();
    let shape = [8i32, 128, 128];
    let numel: usize = shape.iter().map(|&d| d as usize).product();
    let host_y: Vec<bf16> = (0..numel).map(|i| bf16::from_f32(((i % 100) as f32) * 0.0625 + 1.0)).collect();
    ...
    let stride = contiguous_stride(shape);
    let desc = UnaryBackwardDescriptor { kind: UnaryKind::Sqrt, shape, element: ElementKind::Bf16 };
    ...
    for i in 0..numel {
        ...
        assert!((g - exp).abs() <= tol, "sqrt bw bf16 @ {i}: got {g} exp {exp}");
    }
}
