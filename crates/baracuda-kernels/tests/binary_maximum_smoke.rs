...
fn maximum_f32_contig() {
    ...
    for (i, ((&a, &b), &g)) in host_a.iter().zip(host_b.iter()).zip(got.iter()).enumerate() {
        ...
        if want.is_nan() {
            assert!(
                g.is_nan(),
                "f32 maximum @ {i}: a={a} b={b} got={g} want=NaN"
            );
        } else {
            assert_eq!(
                g.to_bits(),
                want.to_bits(),
                "f32 maximum @ {i}: a={a} b={b} got={g} want={want}"
            );
        }
    }
}
...
fn maximum_f64_contig() {
    ...
    for (i, ((&a, &b), &g)) in host_a.iter().zip(host_b.iter()).zip(got.iter()).enumerate() {
        ...
        if want.is_nan() {
            assert!(
                g.is_nan(),
                "f64 maximum @ {i}: a={a} b={b} got={g} want=NaN"
            );
        } else {
            assert_eq!(
                g.to_bits(),
                want.to_bits(),
                "f64 maximum @ {i}: a={a} b={b} got={g} want={want}"
            );
        }
    }
}
...
fn maximum_f16_contig() {
    ...
    for (i, ((&a, &b), &g)) in host_a.iter().zip(host_b.iter()).zip(got.iter()).enumerate() {
        ...
        if want_f.is_nan() {
            assert!(
                g.to_f32().is_nan(),
                "f16 maximum @ {i}: a={a} b={b} got={g} want=NaN"
            );
        } else {
            ...
            assert_eq!(
                g.to_bits(),
                want.to_bits(),
                "f16 maximum @ {i}: a={a} b={b} got={g} want={want}"
            );
        }
    }
}
...
fn maximum_bf16_contig() {
    ...
    for (i, ((&a, &b), &g)) in host_a.iter().zip(host_b.iter()).zip(got.iter()).enumerate() {
        ...
        if want_f.is_nan() {
            assert!(
                g.to_f32().is_nan(),
                "bf16 maximum @ {i}: a={a} b={b} got={g} want=NaN"
            );
        } else {
            ...
            assert_eq!(
                g.to_bits(),
                want.to_bits(),
                "bf16 maximum @ {i}: a={a} b={b} got={g} want={want}"
            );
        }
    }
}
