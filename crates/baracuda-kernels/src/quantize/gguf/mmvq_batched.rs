...
impl<T: GgufMmvqBatchedActivation> GgufMmvqBatchedPlan<T> {
    ...
    pub fn select(
        ...
        if let GgufMmvqBatchedFormat::Quantized(fmt) = desc.format {
            ...
            // #127: the type-0/1 MMVQ kernel reads whole 64-column strides, so
            // `n_cols % 64 == 32` reads 32 columns past `n_cols`. Here the
            // activations are contiguous-batched, so that over-read lands in the
            // next token's row and the result is silently wrong. This replaces a
            // debug-only `n_cols < 64` check, which missed widths like 96 and was
            // skipped in release builds.
            if super::mmvq::uses_dmmv_stride(fmt)
