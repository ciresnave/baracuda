//! BCE loss plan — `y = -mean(target·log(pred) + (1-target)·log(1-pred))`.
//!
//! Caller ensures `pred ∈ (0, 1)`; the kernel does not guard against
//! `log(0)`. Today wired: `Bce × {f32, f16, bf16, f64}` × `{None, Mean, Sum}`.
...
/// Descriptor for a BCE loss op.
#[derive(Copy, Clone, Debug)]
pub struct BceLossDescriptor<const N: usize> {
...
impl<T: Element, const N: usize> BceLossPlan<T, N> {
    /// Pick a kernel.
    pub fn select(
        _stream: &Stream,
        desc: &BceLossDescriptor<N>,
        ...
        if desc.element != T::KIND {
            return Err(Error::Unsupported(
                "baracuda-kernels::BceLossPlan: descriptor element != T",
            ));
        }
    ...
    pub fn run(
        ...
        let status = match T::KIND {
            ElementKind::F32 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_f32_run(
                    numel, mode, pred_ptr, target_ptr, out_ptr, ws_ptr, ws_bytes, stream_ptr,
                )
            },
            ElementKind::F16 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_f16_run(
                    numel, mode, pred_ptr, target_ptr, out_ptr, ws_ptr, ws_bytes, stream_ptr,
                )
            },
            ElementKind::Bf16 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_bf16_run(
                    numel, mode, pred_ptr, target_ptr, out_ptr, ws_ptr, ws_bytes, stream_ptr,
                )
            },
            ElementKind::F64 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_f64_run(
                    numel, mode, pred_ptr, target_ptr, out_ptr, ws_ptr, ws_bytes, stream_ptr,
                )
            },
            _ => {
                return Err(Error::Unsupported(
                    "baracuda-kernels::BceLossPlan::run unwired dtype",
                ));
            }
        };
        map_status(status)
...
/// Descriptor for a BCE backward op.
#[derive(Copy, Clone, Debug)]
pub struct BceLossBackwardDescriptor<const N: usize> {
...
pub struct BceLossBackwardArgs<'a, T: Element, const N: usize> {
    ...
    pub target: TensorRef<'a, T, N>,
    /// Upstream gradient (shape == input for None; scalar for Mean / Sum).
...
impl<T: Element, const N: usize> BceLossBackwardPlan<T, N> {
    /// Pick a kernel.
    pub fn select(
        _stream: &Stream,
        desc: &BceLossBackwardDescriptor<N>,
        ...
        if desc.element != T::KIND {
            return Err(Error::Unsupported(
                "baracuda-kernels::BceLossBackwardPlan: descriptor element != T",
            ));
        }
        check_supported_dtype::<T>()?;
        validate_shape(&desc.input_shape, N)?;
    ...
    pub fn run(
        ...
        args: BceLossBackwardArgs<'_, T, N>,
    ) -> Result<()> {
        ...
        {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::BceLossBackwardPlan: shape mismatch",
            ));
        }
        let numel = args.pred.numel();
        if numel == 0 {
        ...
        let status = match T::KIND {
            ElementKind::F32 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_backward_f32_run(
                    numel, mode, inv_n_or_one, pred_ptr, target_ptr, dy_ptr, dpred_ptr,
                    core::ptr::null_mut(), 0, stream_ptr,
                )
            },
            ElementKind::F16 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_backward_f16_run(
                    numel, mode, inv_n_or_one, pred_ptr, target_ptr, dy_ptr, dpred_ptr,
                    core::ptr::null_mut(), 0, stream_ptr,
                )
            },
            ElementKind::Bf16 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_backward_bf16_run(
                    numel, mode, inv_n_or_one, pred_ptr, target_ptr, dy_ptr, dpred_ptr,
                    core::ptr::null_mut(), 0, stream_ptr,
                )
            },
            ElementKind::F64 => unsafe {
                baracuda_kernels_sys::baracuda_kernels_loss_bce_backward_f64_run(
                    numel, mode, inv_n_or_one, pred_ptr, target_ptr, dy_ptr, dpred_ptr,
                    core::ptr::null_mut(), 0, stream_ptr,
                )
            },
            _ => {
                return Err(Error::Unsupported(
                    "baracuda-kernels::BceLossBackwardPlan::run unwired dtype",
                ));
            }
        };
        map_status(status)
