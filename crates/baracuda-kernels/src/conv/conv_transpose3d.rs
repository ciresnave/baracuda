//! ConvTranspose3d — 3-D transposed convolution via cuDNN.
//!
//! Implementation strategy mirrors [`super::conv_transpose2d`] /
//! [`super::conv_transpose1d`]: cuDNN has no direct transpose entry
//! point, so we configure descriptors for a synthetic dense Conv3d
//! that maps `[N, c_out, D_out, H_out, W_out] → [N, c_in, D_in, H_in,
//! W_in]`, then call `cudnnConvolutionBackwardData` for the
//! ConvTranspose forward and `cudnnConvolutionForward` for its
//! backward-data. Filter shape follows PyTorch: `[C_in, C_out /
//! groups, D_filt, H_filt, W_filt]`.
...
/// Descriptor for a 3-D transposed convolution over NCDHW tensors.
///
...
#[non_exhaustive]
pub struct ConvTranspose3dDescriptor {
    /// Batch size `N`.
    pub batch: i32,
    ...
    /// Stride along W.
    pub stride_w: i32,
    /// Dilation along D / H / W.
    pub dilation_d: i32,
    /// Dilation along H.
    pub dilation_h: i32,
    /// Dilation along W.
    pub dilation_w: i32,
    /// Output-side padding along D.
    pub output_pad_d: i32,
    /// Output-side padding along H.
...
impl ConvTranspose3dDescriptor {
    /// Build a descriptor with `pad / stride / dilation / output_pad /
    /// groups` defaulted to PyTorch's `nn.ConvTranspose3d` defaults
    ...
    /// Override `(output_pad_d, output_pad_h, output_pad_w)`. Default
    /// `(0, 0, 0)`.
    #[inline]
    pub fn with_output_padding(
        ...
        self.output_pad_d = output_pad_d;
...
/// Args bundle for a ConvTranspose3d forward launch.
pub struct ConvTranspose3dArgs<'a, T: Element> {
    /// Input activations `[N, C_in, D_in, H_in, W_in]`.
    pub x: TensorRef<'a, T, 5>,
    /// Filter weights `[C_in, C_out / groups, D_filt, H_filt, W_filt]`.
    pub w: TensorRef<'a, T, 5>,
    /// Output activations `[N, C_out, D_out, H_out, W_out]`.
    pub y: TensorMut<'a, T, 5>,
}

/// Args bundle for a ConvTranspose3d data-gradient launch.
pub struct ConvTranspose3dBwArgs<'a, T: Element> {
    /// Filter weights `[C_in, C_out / groups, D_filt, H_filt, W_filt]`.
    pub w: TensorRef<'a, T, 5>,
...
/// Args bundle for a ConvTranspose3d filter-gradient launch.
pub struct ConvTranspose3dDwArgs<'a, T: Element> {
    /// Input activations `[N, C_in, D_in, H_in, W_in]`.
    pub x: TensorRef<'a, T, 5>,
    /// Upstream gradient `[N, C_out, D_out, H_out, W_out]`.
    pub dy: TensorRef<'a, T, 5>,
    /// Output gradient w.r.t. the filter
    /// `[C_in, C_out / groups, D_filt, H_filt, W_filt]`.
    pub dw: TensorMut<'a, T, 5>,
}

/// 3-D transposed-convolution plan (cuDNN-backed). See the module
/// docs for the role-swap strategy.
pub struct ConvTranspose3dPlan<T: Element> {
    desc: ConvTranspose3dDescriptor,
    sku: KernelSku,
...
impl<T: Element> ConvTranspose3dPlan<T> {
    ...
    pub fn select(
        ...
        if desc.element != T::KIND {
            return Err(Error::Unsupported(
                "baracuda-kernels::ConvTranspose3dPlan: descriptor.element != T::KIND",
            ));
        }
        if !matches!(
            T::KIND,
            ...
            return Err(Error::Unsupported(
                "baracuda-kernels::ConvTranspose3dPlan: dtype must be f32 / f64 / f16 / bf16",
            ));
        }
        if desc.batch <= 0
            || desc.c_in <= 0
            ...
            || desc.w_in <= 0
        {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: input shape extents must be > 0",
            ));
        }
        if desc.c_out <= 0 || desc.d_filt <= 0 || desc.h_filt <= 0 || desc.w_filt <= 0 {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: filter shape extents must be > 0",
            ));
        }
        if desc.stride_d <= 0
            || desc.stride_h <= 0
        ...
        {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: stride / dilation must be > 0",
            ));
        }
        if desc.pad_d < 0
            || desc.pad_h < 0
        ...
        {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: padding / output_padding must be >= 0",
            ));
        }
        if desc.groups <= 0 {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: groups must be > 0",
            ));
        }
        if desc.c_in % desc.groups != 0 || desc.c_out % desc.groups != 0 {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: groups must divide both c_in and c_out",
            ));
        }
        if desc.output_pad_d >= desc.stride_d.max(desc.dilation_d)
            || desc.output_pad_h >= desc.stride_h.max(desc.dilation_h)
            || desc.output_pad_w >= desc.stride_w.max(desc.dilation_w)
        {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: output_padding must be < max(stride, dilation)",
            ));
        }
        let (d_out, h_out, w_out) = compute_output_dims(desc);
        if d_out <= 0 || h_out <= 0 || w_out <= 0 {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: computed output dims <= 0",
            ));
        }
        ...
        let sku = KernelSku {
            ...
            op: ConvKind::ConvTranspose3d as u16,
            element: T::KIND,
            aux_element: None,
            layout: None,
            epilogue: None,
    ...
    /// Run the forward pass — `cudnnConvolutionBackwardData` with the
    /// role swap.
    pub fn run_fw(
        &self,
        stream: &Stream,
        workspace: Workspace<'_>,
        args: ConvTranspose3dArgs<'_, T>,
    ) -> Result<()> {
    ...
    /// Run the data-gradient pass — `cudnnConvolutionForward` with
    /// the role swap.
    pub fn run_bw_data(
        &self,
        stream: &Stream,
        workspace: Workspace<'_>,
        args: ConvTranspose3dBwArgs<'_, T>,
    ) -> Result<()> {
    ...
    pub fn run_dw(
        ...
        stream: &Stream,
        workspace: Workspace<'_>,
        args: ConvTranspose3dDwArgs<'_, T>,
    ) -> Result<()> {
    ...
    fn ensure_descriptors(&self) -> Result<()> {
        ...
        // synth_x: [N, C_out, D_out, H_out, W_out].
        let mut xd: cudnnTensorDescriptor_t = core::ptr::null_mut();
        let status = unsafe { cudnnCreateTensorDescriptor(&mut xd as *mut _) };
        if status != 0 {
            return Err(Error::CutlassInternal(-status));
        }
        let x_dims = [self.desc.batch, self.desc.c_out, d_out, h_out, w_out];
        let x_s_w = 1;
        let x_s_h = w_out;
        let x_s_d = h_out * w_out;
        let x_s_c = d_out * h_out * w_out;
        let x_s_n = self.desc.c_out * x_s_c;
        let x_strides = [x_s_n, x_s_c, x_s_d, x_s_h, x_s_w];
        let status = unsafe {
        ...
        let y_s_w = 1;
        let y_s_h = self.desc.w_in;
        let y_s_d = self.desc.h_in * self.desc.w_in;
        let y_s_c = self.desc.d_in * self.desc.h_in * self.desc.w_in;
        let y_s_n = self.desc.c_in * y_s_c;
        ...
        // synth_w: [C_in, C_out/groups, D, H, W].
        ...
        // Conv descriptor — array_length = 3.
    ...
    fn check_fw_args(&self, args: &ConvTranspose3dArgs<'_, T>) -> Result<()> {
        let (d_out, h_out, w_out) = compute_output_dims(&self.desc);
        let c_out_per_group = self.desc.c_out / self.desc.groups;
        let x_shape = [
            self.desc.batch,
        ...
        if args.x.shape != x_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: x shape != [N, C_in, D_in, H_in, W_in]",
            ));
        }
        if args.w.shape != w_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: w shape != [C_in, C_out/groups, D_filt, H_filt, W_filt]",
            ));
        }
        if args.y.shape != y_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: y shape != [N, C_out, D_out, H_out, W_out]",
            ));
        }
        Ok(())
    }

    fn check_bw_data_args(&self, args: &ConvTranspose3dBwArgs<'_, T>) -> Result<()> {
        let (d_out, h_out, w_out) = compute_output_dims(&self.desc);
        let c_out_per_group = self.desc.c_out / self.desc.groups;
        let w_shape = [
            self.desc.c_in,
        ...
        if args.dy.shape != dy_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: dy shape != [N, C_out, D_out, H_out, W_out]",
            ));
        }
        if args.dx.shape != dx_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: dx shape != [N, C_in, D_in, H_in, W_in]",
            ));
        }
        Ok(())
    }

    fn check_dw_args(&self, args: &ConvTranspose3dDwArgs<'_, T>) -> Result<()> {
        let (d_out, h_out, w_out) = compute_output_dims(&self.desc);
        let c_out_per_group = self.desc.c_out / self.desc.groups;
        let x_shape = [
            self.desc.batch,
        ...
        if args.dw.shape != dw_shape {
            return Err(Error::InvalidProblem(
                "baracuda-kernels::ConvTranspose3dPlan: dw shape != [C_in, C_out/groups, D_filt, H_filt, W_filt]",
            ));
        }
        Ok(())
    }
...
impl<T: Element> Drop for ConvTranspose3dPlan<T> {
    fn drop(&mut self) {
...
#[inline]
fn compute_output_dims(d: &ConvTranspose3dDescriptor) -> (i32, i32, i32) {
    let d_out = (d.d_in - 1) * d.stride_d - 2 * d.pad_d
        + d.dilation_d * (d.d_filt - 1)
        + d.output_pad_d
        + 1;
...
fn cudnn_dtype<T: Element>() -> i32 {
    match T::KIND {
        ...
        _ => unreachable!("ConvTranspose3dPlan::select gates on F32/F64/F16/Bf16"),
    }
}
