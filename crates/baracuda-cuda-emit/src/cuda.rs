...
//! math itself is lowered by the language-neutral [`unpopped::backend::lower_expr`]
...
impl Backend for Cuda {
    ...
    fn lower(&self, plan: &KernelPlan<'_>) -> Result<GeneratedKernel, LowerError> {
        (|| -> Result<GeneratedKernel, LowerError> {
            ...
            match plan.schedule {
                ...
                Schedule::Strided => {
                    ...
                    match plan.write_index.scatter() {
                        Some((_, _, combine, _, _)) if combine.is_fp_atomic_add(plan.out_dtype) => {
                            Ok(emit_scatter_gathersum(plan, ctype))
                        }
                ...
                Schedule::RowSort { .. } => Ok(emit_row_sort(plan, ctype)),
                ...
                Schedule::Im2Col { .. } => Ok(emit_im2col(plan, ctype)),
...
fn cuda_lowering<'a>(
    ...
    reduced: &'a dyn Fn(u8) -> Result<Spelling, LowerError>,
    coord: &'a dyn Fn(u8) -> Result<Spelling, LowerError>,
    ...
    select: &'a dyn Fn(String, String, String) -> Result<Spelling, LowerError>,
) -> Lowering<'a> {
...
/// ([`unpopped::backend::lower_dag_all`]), so the `{x}` duplication below never
/// duplicates a computation.
fn packed_unary(op: UnaryOp, x: String, dt: ElementKind) -> Result<Spelling, LowerError> {
    Ok(match op {
        UnaryOp::Neg => Spelling::Spelled(format!("__hneg2({x})")),
        UnaryOp::Abs => Spelling::Spelled(format!("__habs2({x})")),
        ...
        _ => {
            ...
            match (l.spelled(), h.spelled()) {
                (Ok(l), Ok(h)) => Spelling::Spelled(format!("{join}({l}, {h})")),
                // Preserve a decline from either half — never fabricate a join
                // over an unspelled operand.
                (Err(d), _) | (_, Err(d)) => Spelling::Declined(d),
            }
        }
    })
}
...
fn packed_binary(
    ...
    Ok(match (l.spelled(), h.spelled()) {
        (Ok(l), Ok(h)) => Spelling::Spelled(format!("{join}({l}, {h})")),
        // Preserve a decline from either half — never fabricate a join over an
        // unspelled operand.
        (Err(d), _) | (_, Err(d)) => Spelling::Declined(d),
    })
}
                    ...
                    Ok(Spelling::Declined(Decline::UnsupportedOp {
                        ...
                        why: format!(
                            "cuda backend: Coord({d}) reached the vectorized emitter — Coord \
                             ...
                             per-axis coordinates) — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }))
                },
    ...
    for field in pk.fields {
        ...
        let (prelude, root) = lower_dag_all(
            ...
            &cuda_lowering(
                &acc,
                &reduced_decline_no_rowreduce,
                // Doubly plan-gate-invariant: `body_packs` is Input-leaf-only AND
                // the packed dtypes are outside Coord's f32/f64 gate.
                &|d| {
                    Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Coord(d),
                        why: format!(
                            "cuda backend: Coord({d}) reached the packed emitter — Coord \
                             ...
                             Coord dtype gate) — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }))
                },
                ...
                &|_, _, _| {
                    Ok(Spelling::Declined(Decline::UnsupportedOp {
                        ...
                        why: "cuda backend: Select reached the packed emitter — body_packs \
                              excludes select bodies (they decline to the scalar path) — \
                              plan-gate invariant, unpopped 0.5.0"
                            .to_string(),
                    }))
                },
            ),
...
fn emit_scalar(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
    let name = format!(
                ...
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    ...
                    why: format!(
                        "cuda backend: Coord({d}) reached the scalar emitter — Coord bodies \
                         ...
                         coordinates) — plan-gate invariant, unpopped 0.5.0"
                    ),
                }))
            },
            &|op, x| cuda_unary(op, x, plan.dtype, bit_move),
...
fn emit_strided(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
    ...
    let gather = unpopped::plan::gather_of(plan.read_index);
    ...
    let scatter = unpopped::plan::scatter_of(plan.write_index);
    ...
    if let Some((g, idx_op, axis, oob, _idt)) = gather {
        ...
        if matches!(
            oob,
            unpopped::ir::OobPolicy::Skip | unpopped::ir::OobPolicy::ZeroFill
        ) {
    ...
    let acc = |idx: u8| -> Result<Spelling, LowerError> {
        Ok(Spelling::Spelled(
    ...
    // axis to `< rank` before this closure can run. The former assert!/panic!
    // invariants are now typed declines (KISS-EMIT §6.8-0004: no panic on any
    // input, and the emitter cannot verify the "backstop refused this" claim).
    let coord = |d: u8| -> Result<Spelling, LowerError> {
        if (d as usize) >= rank {
            return Ok(Spelling::Declined(Decline::UnsupportedOp {
                op: DeclinedOp::Coord(d),
                why: format!(
                    "cuda backend: Coord({d}) axis out of range for the rank-{rank} strided \
                     unravel (the backstop should have refused this plan) — plan-gate \
                     invariant, unpopped 0.5.0"
                ),
            }));
        }
        Ok(Spelling::Spelled(match plan.dtype {
    ...
    if let Some((_, _, combine, _, _)) = scatter {
        ...
        match gather {
            Some((_, _, _, unpopped::ir::OobPolicy::Skip, _)) => {
                s.push_str(&format!("        if (!goob) out[oo] = {stored};\n"));
            }
            Some((_, _, _, unpopped::ir::OobPolicy::ZeroFill, _)) => {
...
fn scatter_combine_store(
    combine: unpopped::ir::WriteCombine,
    octype: &str,
    root: &str,
    hetero: bool,
) -> String {
    use unpopped::ir::WriteCombine;
    match combine {
        WriteCombine::Assign => {
            let val = if hetero {
                format!("({octype})({root})")
...
fn assert_gather_lowerable(plan: &KernelPlan<'_>) {
    if unpopped::plan::gather_of(plan.read_index).is_none() {
    ...
    let (g, index_operand, axis, _oob, index_dtype) =
        unpopped::plan::gather_of(plan.read_index).expect("gather present (checked above)");
    assert!(
...
fn assert_scatter_lowerable(plan: &KernelPlan<'_>) {
    ...
    assert!(
        unpopped::plan::gather_of(plan.read_index).is_none(),
    ...
    assert!(
        unpopped::plan::combine_legal_for_dtype(combine, plan.out_dtype),
...
/// indices carrying a [`unpopped::ir::BaseOffset::Runtime`] offset, and whether the
...
fn scatter_atomic_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    // A lowering decline/error ⇒ don't offer the variant (never-worse: the
    // deterministic gather-sum base still serves the cell).
    let atomic = emit_strided(plan, ctype).ok()?;
...
fn assert_views_lowerable(plan: &KernelPlan<'_>) {
    if !plan.views.iter().any(unpopped::plan::view_is_addressing) {
    ...
    for (i, v) in plan.views.iter().enumerate() {
        ...
        match v {
            ...
            unpopped::ir::View::Reshape { producer_rank } => assert!(
                *producer_rank == rank,
...
/// Declining `reduced`/`coord` closures for the multi-output elementwise
/// emitters: v1 multi-output bodies carry neither leaf (rejected at the plan
/// gate + emitter backstops), so reaching one is a plan-gate-invariant miss.
...
fn multi_reduced_decline(op_name: &str) -> impl Fn(u8) -> Result<Spelling, LowerError> + '_ {
    move |i| {
        Ok(Spelling::Declined(Decline::UnsupportedOp {
            ...
            why: format!(
                "cuda backend: Reduced({i}) in multi-output op '{op_name}' — multi-output v1 is elementwise-map only (no reduction) — plan-gate invariant, unpopped 0.5.0"
            ),
        }))
    }
}
fn multi_coord_decline(op_name: &str) -> impl Fn(u8) -> Result<Spelling, LowerError> + '_ {
    move |d| {
        Ok(Spelling::Declined(Decline::UnsupportedOp {
            op: DeclinedOp::Coord(d),
            why: format!(
                "cuda backend: Coord({d}) in multi-output op '{op_name}' — multi-output v1 is elementwise-map only (Coord bodies are deferred) — plan-gate invariant, unpopped 0.5.0"
            ),
        }))
    }
}

/// The `reduced` seam for every emitter EXCEPT the RowReduce path: only a
/// row-reduction body contains a [`ScalarExpr::Reduced`] leaf, so reaching this
/// elsewhere is a plan-gate-invariant miss. USED TO `unreachable!`; per
/// KISS-EMIT §6.8-0004 the emitter cannot verify the "unreachable" claim across
/// the crate boundary, so it returns a typed decline naming the inherited
/// invariant (a caller may still treat it as fatal).
fn reduced_decline_no_rowreduce(i: u8) -> Result<Spelling, LowerError> {
    Ok(Spelling::Declined(Decline::UnsupportedOp {
        op: DeclinedOp::Reduced(i),
        why: format!(
            "no Reduced leaf outside RowReduce: red{i} — plan-gate invariant, unpopped 0.5.0"
        ),
    }))
}
...
/// ([`unpopped::ir::ExprDag::from_exprs`]) so a value shared between outputs — the
...
fn emit_scalar_multi(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
...
fn emit_strided_multi(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
    let rank = plan.key.rank as usize;
    let n_in = plan.n_inputs as usize;
    let n_out = plan.n_outputs as usize;
    let name = format!(
...
fn emit_vectorized_packed_multi(
    plan: &KernelPlan<'_>,
    pk: &PackedKind,
) -> Result<GeneratedKernel, LowerError> {
        ...
        let (prelude, roots) = lower_dag_multi(
            ...
            &cuda_lowering(
                ...
                &|_, _, _| {
                    Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Select,
                        why: "cuda backend: Select reached the packed multi-output emitter — \
                              body_packs excludes select bodies (they decline to the scalar \
                              path) — plan-gate invariant, unpopped 0.5.0"
                            .to_string(),
                    }))
                },
            ),
...
fn emit_reduction(
    ...
    // `load` is a String speller shared with the int-predicate path below, so it
    // stays `-> String` and is wrapped into the seam's `Ok(Spelling::Spelled(..))`
    // here. A decline surfaced by `lower_expr` becomes this emitter's typed
    // LowerError via `?.spelled()?` (the `?` on `spelled()`'s `Decline` uses
    // `From<Decline> for LowerError`).
    let elem = match int_reduction_predicate(plan.body, &load, &|i| {
    ...
    // `post_apply` now returns a `Result`: a decline from lowering the post-expr
    // becomes the emitter's typed `LowerError` at the call site's `?`. `post_leaf`
    // / `post_reduced` stay `-> String` (shared with the int-predicate path) and
    // are lifted into the seams here; `post_reduced` is a real spelling (`red0`).
    let post_apply = |finalized: String| -> Result<(Option<String>, String), LowerError> {
        if post_is_identity {
            return Ok((None, store(finalized)));
        }
        ...
        let posted = match int_reduction_predicate(post, &post_leaf, &post_reduced) {
            Some(s) => s,
            None => lower_expr(
                post,
                &cuda_lowering(
                    // A post-expr Input is a plan-gate-invariant miss (validated
                    // away) — a typed decline through the seam, not the panic the
                    // String `post_leaf` still carries on the int-predicate path.
                    &|i| {
                        Ok(Spelling::Declined(Decline::UnsupportedOp {
                            op: DeclinedOp::Leaf(i),
                            why: format!(
                                "cuda backend: reduction post-expr Input({i}) reached the \
                                     emitter — the post reads Reduced(0)/Const/Param only \
                                     (validated by plan::assert_valid_reduction_post) — \
                                     plan-gate invariant, unpopped 0.5.0"
                            ),
                        }))
                    },
                    // Only Reduced(0) (the fold result `red0`) exists — a higher
                    // index is a plan-gate-invariant miss (the String `post_reduced`
                    // still `assert_eq!`s it on the int-predicate path); a typed
                    // decline through the seam.
                    &|s| {
                        if s != 0 {
                            return Ok(Spelling::Declined(Decline::UnsupportedOp {
                                op: DeclinedOp::Reduced(s),
                                why: format!(
                                    "cuda backend: reduction post references Reduced({s}); \
                                     only 0 exists — plan-gate invariant, unpopped 0.5.0"
                                ),
                            }));
                        }
                        Ok(Spelling::Spelled("red0".to_string()))
                    },
                    &|d| {
                        Ok(Spelling::Declined(Decline::UnsupportedOp {
                            op: DeclinedOp::Coord(d),
                            why: format!(
                                "cuda backend: reduction post-expr Coord({d}) is \
                                     Elementwise-only — plan-gate invariant, unpopped 0.5.0"
                            ),
                        }))
                    },
                    &|op, x| {
        ...
        Ok((Some(format!("{acc} red0 = {finalized};")), store(posted)))
    };
    ...
    if class == ReduceAxisClass::InnerContig && !precision {
        ...
        s.push_str("    }\n}\n");
        return Ok(GeneratedKernel::new(name, s));
    }
...
fn precision_first_reduction(plan: &KernelPlan<'_>, rop: ReduceOp) -> Option<Variant> {
    ...
    let ctype = scalar_ctype(plan.dtype)?;
    // A lowering decline/error ⇒ don't offer the variant (never-worse: the base
    // reduction still serves the cell).
    let kernel = emit_reduction(plan, ctype, rop, true).ok()?;
...
fn precision_first_rowreduce(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    let ctype = scalar_ctype(plan.dtype)?;
    // A lowering decline/error ⇒ don't offer the variant (never-worse).
    let kernel = emit_row_reduce_impl(plan, ctype, false, true).ok()?;
    let det = VariantFidelity::MorePrecise.determinism_str();
    Some(Variant::new(
        "prec",
        vec![kernel],
...
fn reduction_splitk_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    // A lowering decline/error ⇒ don't offer the variant (never-worse: the base
    // reduction path still serves the cell). `load` is a local String speller,
    // lifted into the seam here.
    let elem = lower_expr(
        plan.body,
        &cuda_lowering(
            &|i| Ok(Spelling::Spelled(load(i))),
                ...
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    ...
                    why: format!(
                        "cuda backend: Coord({d}) reached the split-K reduction variant — \
                         Coord is Elementwise-only — plan-gate invariant, unpopped 0.5.0"
                    ),
                }))
            },
            &|op, x| {
...
fn emit_contraction(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
    ...
    // (`unpopped_vocab::structure_key`'s `contraction_acc`, mirroring
    ...
    let epi = lower_expr(
        ...
        &cuda_lowering(
            ...
            // accumulator width, exactly as the operand loads are. A sub-2 leaf is
            // a plan-gate-invariant miss (was an `assert!`), now a typed decline.
            &|i| {
                if i < 2 {
                    return Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Leaf(i),
                        why: format!(
                            "contraction epilogue Input leaf must be the fused bias (>=2): \
                             in{i} — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }));
                }
            ...
            // Only Reduced(0) (the accumulator `r0`) exists — a higher index is a
            // plan-gate-invariant miss (was an `assert_eq!`), now a typed decline.
            &|s| {
                if s != 0 {
                    return Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Reduced(s),
                        why: format!(
                            "cuda backend: contraction epilogue reads Reduced(0) only; got \
                             Reduced({s}) — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }));
                }
                Ok(Spelling::Spelled("r0".to_string()))
            },
            &|d| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Coord(d),
...
fn contraction_splitk_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    if c.m != unpopped_vocab::SizeClass::Tiny {
        return None;
    }
    ...
    // A lowering decline/error ⇒ don't offer the variant (never-worse: the base
    // contraction path still serves the cell).
    let epi = lower_expr(
        epilogue,
        &cuda_lowering(
            &|i| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Leaf(i),
                    why: format!(
                        "contraction v1 epilogue has no Input leaf: in{i} — plan-gate \
                         invariant, unpopped 0.5.0"
                    ),
                }))
            },
...
/// One-letter tag for a [`unpopped_vocab::SizeClass`] in symbol names.
fn size_tag(s: unpopped_vocab::SizeClass) -> char {
    match s {
        unpopped_vocab::SizeClass::Tiny => 't',
        unpopped_vocab::SizeClass::Small => 's',
        unpopped_vocab::SizeClass::Mid => 'm',
        unpopped_vocab::SizeClass::Large => 'l',
    }
}
...
fn emit_row_reduce(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
...
fn row_reduce_materialize_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    // A lowering decline/error ⇒ don't offer the variant (never-worse).
    let k = emit_row_reduce_impl(plan, ctype, true, false).ok()?;
...
fn emit_row_reduce_impl(
    ...
    // `load`/`red` are String spellers, lifted into the seams here; a decline
    // from lowering becomes this emitter's typed LowerError via `?.spelled()?`.
    let lower = |e: &ScalarExpr| -> Result<String, LowerError> {
        Ok(lower_expr(
            e,
            &cuda_lowering(
                &|i| Ok(Spelling::Spelled(load(i))),
                &|s| Ok(Spelling::Spelled(red(s))),
                &|d| {
                    Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Coord(d),
                        why: format!(
                            "cuda backend: Coord({d}) reached the RowReduce emitter — the \
                             ...
                             is Elementwise-only — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }))
                },
                &|op, x| {
...
/// `INFINITY` FP extremes are already emitted by [`unpopped::backend::const_lit`], so
...
/// The serial-fold scan BASE (`block = false`) — [`unpopped::backend::VariantFidelity::BitIdentical`].
fn emit_scan(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
...
/// block-scan VARIANT (`block = true`). See [`unpopped::ir::Access::Scan`] and §3/§4
...
fn emit_scan_impl(
        ...
        &cuda_lowering(
            ...
            &|s| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Reduced(s),
                    why: format!(
                        "cuda backend: Scan pre-map read Reduced({s}) — no running prefix in \
                         the pre-map — plan-gate invariant, unpopped 0.5.0"
                    ),
                }))
            },
            &|d| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Coord(d),
                    why: format!(
                        "cuda backend: Scan Coord({d}) is Elementwise-only — plan-gate \
                         invariant, unpopped 0.5.0"
                    ),
                }))
            },
    ...
    let post_str = lower_expr(
        ...
        &cuda_lowering(
            &|i| Ok(Spelling::Spelled(load(i))),
            &|s| {
                // Only Reduced(0) (the running prefix) exists — a higher index is a
                // plan-gate-invariant miss (was an `assert_eq!`), now a typed decline.
                if s != 0 {
                    return Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Reduced(s),
                        why: format!(
                            "cuda backend: Scan post references Reduced({s}); only 0 (the \
                             running prefix) exists — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }));
                }
                Ok(Spelling::Spelled("prefix".to_string()))
            },
            &|d| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Coord(d),
...
fn scan_blockscan_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    let ctype = scalar_ctype(plan.dtype)?;
    // A lowering decline/error ⇒ don't offer the variant (never-worse).
    let k = emit_scan_impl(plan, ctype, true).ok()?;
...
/// [`unpopped::backend::VariantFidelity::BitIdentical`]. One thread per OUTPUT
...
fn emit_window(plan: &KernelPlan<'_>, ctype: &str) -> Result<GeneratedKernel, LowerError> {
    let (pre, post) = match plan.access {
        ...
        &cuda_lowering(
            ...
            &|s| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Reduced(s),
                    why: format!(
                        "cuda backend: Window pre-map read Reduced({s}) — no window result in \
                         the pre-map — plan-gate invariant, unpopped 0.5.0"
                    ),
                }))
            },
            &|d| {
                Ok(Spelling::Declined(Decline::UnsupportedOp {
                    op: DeclinedOp::Coord(d),
                    why: format!(
                        "cuda backend: Window Coord({d}) is Elementwise-only — plan-gate \
                         invariant, unpopped 0.5.0"
                    ),
                }))
            },
    ...
    let post_str = lower_expr(
        ...
        &cuda_lowering(
            &|i| Ok(Spelling::Spelled(load(i))),
            &|s| {
                // Only Reduced(0) (the window result) exists — a higher index is a
                // plan-gate-invariant miss (was an `assert_eq!`), now a typed decline.
                if s != 0 {
                    return Ok(Spelling::Declined(Decline::UnsupportedOp {
                        op: DeclinedOp::Reduced(s),
                        why: format!(
                            "cuda backend: Window post references Reduced({s}); only 0 (the \
                             window result) exists — plan-gate invariant, unpopped 0.5.0"
                        ),
                    }));
                }
                Ok(Spelling::Spelled("prefix".to_string()))
...
/// [`unpopped::backend::VariantFidelity::BitIdentical`]. One thread per OUTPUT cell
...
/// [`unpopped::backend::VariantFidelity::BitIdentical`], any `k`, no smem/barriers.
...
/// [`unpopped::ir::Access::RowSort`] and §4 of the increment-8 brief.
///
...
fn emit_row_sort_impl(plan: &KernelPlan<'_>, ctype: &str, bitonic: bool) -> GeneratedKernel {
    ...
    let topk = matches!(limit, unpopped::ir::SortLimit::TopK);
    ...
    let limit_suf = match limit {
        unpopped::ir::SortLimit::Full => "",
        unpopped::ir::SortLimit::TopK => "_topk",
    };
...
fn emit_row_sort_partial(plan: &KernelPlan<'_>, ctype: &str) -> GeneratedKernel {
    ...
    assert!(
        matches!(limit, unpopped::ir::SortLimit::TopK),
...
fn row_sort_bitonic_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    let launch_note = match limit {
        unpopped::ir::SortLimit::Full => format!(
        ...
        unpopped::ir::SortLimit::TopK => format!(
...
fn partial_select_topk_variant(plan: &KernelPlan<'_>) -> Option<Variant> {
    ...
    if !matches!(limit, unpopped::ir::SortLimit::TopK) {
        return None;
    }
...
///
/// `perm` (item 01) is the [`unpopped::ir::View::Permute`] this input is read
...
pub fn emit_coord_unravel_helper(max_rank: usize) -> String {
    let mut s = String::new();
    s.push_str("// @generated by baracuda-cuda-emit — do not edit.\n");
...
/// Only a [`unpopped::ir::View::Permute`] remaps stride indices in [`offset_expr`];
...
fn input_perm<'a>(plan: &'a KernelPlan<'_>, k: usize) -> Option<&'a [u8]> {
    match plan.views.get(k) {
        Some(unpopped::ir::View::Permute { perm }) => Some(perm.as_slice()),
        _ => None,
    }
}
...
fn cuda_unary(
    ...
    Ok(Spelling::Spelled(match dtype {
        ...
        // DTYPE decline (was a panic): this op has no lowering at this dtype. An
        // honest capability statement — the JIT gates it, an AOT author declared
        // it — never a fall-through to wrong math.
        other => {
            return Ok(Spelling::Declined(Decline::UnsupportedDtypeForOp {
                op: DeclinedOp::Unary(op),
                dtype: other,
                why: format!("cuda backend: no unary math for dtype {other:?}"),
            }));
        }
    }))
}
...
fn cuda_binary(
    ...
    Ok(Spelling::Spelled(match dtype {
        ...
        // DTYPE decline (was a panic): honest capability statement.
        other => {
            return Ok(Spelling::Declined(Decline::UnsupportedDtypeForOp {
                op: DeclinedOp::Binary(op),
                dtype: other,
                why: format!("cuda backend: no binary math for dtype {other:?}"),
            }));
        }
    }))
}
...
fn cuda_select(
    ...
    Ok(Spelling::Spelled(match dtype {
        ...
        // DTYPE decline (was a panic): v1 select is float-only; an int select
        // misses honestly at the plan gate.
        other => {
            return Ok(Spelling::Declined(Decline::UnsupportedDtypeForOp {
                op: DeclinedOp::Select,
                dtype: other,
...
/// the plan gate's.
fn assert_coord_lowerable(e: &ScalarExpr, plan: &KernelPlan<'_>) -> Result<(), LowerError> {
    // The three former `assert!`s are now typed declines (KISS-EMIT §6.8-0004: no
    // panic on any input, and the emitter cannot verify the "plan gate rejects
    // this" claim across the crate boundary). Each names the inherited plan-gate
    // invariant it enforces. A decline here is terminal for the whole lowering, so
    // it is surfaced as this emitter's `LowerError` via `From<Decline>` at the `?`.
    match e {
        ScalarExpr::Input(_)
        | ScalarExpr::Const(_)
        | ScalarExpr::Param(_)
        | ScalarExpr::Reduced(_) => {}
        ScalarExpr::Coord(d) => {
            if !matches!(plan.access, Access::Elementwise) {
                return Err(Decline::UnsupportedOp {
                    op: DeclinedOp::Coord(*d),
                    why: format!(
                        "cuda backend: Coord({d}) under a non-Elementwise access — the \
                         reduction-class emitters iterate fold/row/contraction coordinate \
                         spaces, not the elementwise output space; the plan gate rejects \
                         this — plan-gate invariant, unpopped 0.5.0"
                    ),
                }
                .into());
            }
            if !matches!(
                ...
                return Err(Decline::UnsupportedDtypeForOp {
                    op: DeclinedOp::Coord(*d),
                    dtype: plan.dtype,
                    why: format!(
                        "cuda backend: Coord({d}) at non-float dtype {:?} — the coordinate is \
                         ...
                         gate rejects this — plan-gate invariant, unpopped 0.5.0",
                        plan.dtype
                    ),
                }
                .into());
                ...
                return Err(Decline::UnsupportedOp {
                    ...
                    why: format!(
                        "cuda backend: Coord({d}) axis out of range for rank {} — no c{d} \
                         coordinate exists to read; the plan gate rejects this — plan-gate \
                         invariant, unpopped 0.5.0",
                        plan.key.rank
                    ),
                }
...
mod tests {
    ...
    use unpopped::ir::{OpDef, input, param};
    ...
    fn binary_key(dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn gather_2d_key() -> unpopped_vocab::StructureKey {
        gather_2d_key_idx(ElementKind::I32)
    }

    fn gather_2d_key_idx(idx_dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn gather_forces_strided_off_the_vectorized_path() {
        use unpopped::ir::OobPolicy;
        use unpopped::plan::{Schedule, build_plan};
    ...
    fn scatter_2d_key(idx_dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn integer_scatter_add_is_the_deterministic_atomicadd_base() {
        ...
        let vs = unpopped::generate_variants(&op, &key, &Cuda);
        assert_eq!(
            vs.len(),
            1,
            "integer scatter_add ships one unconditional base"
        );
        assert_eq!(vs[0].fidelity, unpopped::VariantFidelity::BitIdentical);
    }
    ...
    fn bincount_scatters_const1_atomicadd_into_i32_counts() {
        ...
        assert_eq!(unpopped::generate_variants(&op, &key, &Cuda).len(), 1);
    }
    ...
    fn fp_scatter_add_offers_the_nondeterministic_atomic_variant() {
        ...
        let op = OpDef::scatter_add("scatter_add", &[ElementKind::F32], 0, ElementKind::I32);
        let vs = unpopped::generate_variants(&op, &scatter_2d_key(ElementKind::I32), &Cuda);
        ...
        assert_eq!(atomic.fidelity, unpopped::VariantFidelity::Nondeterministic);
        assert!(
            atomic.kernels[0]
                .source
    ...
    fn non_scatter_op_emits_byte_identical_to_pre_increment5() {
        ...
        let free = generate(&add_op(&[ElementKind::F32]), &key, &Cuda);
        let with = OpDef::elementwise("add", 2, &[ElementKind::F32], input(0) + input(1))
            .with_scatter(unpopped::ir::WriteIndex::Direct);
        let k = generate(&with, &key, &Cuda);
        assert_eq!(free.name, k.name);
        assert_eq!(
            free.source, k.source,
    ...
    fn scattered_op_on_a_non_strided_schedule_is_refused_by_the_backstop() {
        ...
        use unpopped::ir::{OobPolicy, WriteCombine, WriteIndex};
        ...
        let plan = KernelPlan {
            ...
            extra_out_bodies: &[],
            extra_out_dtypes: &[],
            access: &unpopped::ir::Access::Elementwise,
            views: &[],
            read_index: &[],
            write_index: &wi,
            base_offsets: &[],
    ...
    fn gathered_op_on_a_non_strided_schedule_is_refused_by_the_backstop() {
        use unpopped::backend::Backend;
        use unpopped::ir::{OobPolicy, ReadIndex};
        use unpopped::plan::{KernelPlan, Schedule};
        ...
        let plan = KernelPlan {
            ...
            views: &[],
            read_index: &ri,
            write_index: &unpopped::ir::WriteIndex::Direct,
            base_offsets: &[],
            out_base_offset: unpopped::ir::BaseOffset::Zero,
        };
        let _ = Cuda.lower(&plan).unwrap();
...
{src}"
    ...
    fn precision_first_variant_offered_for_f32_sum() {
        use unpopped::ir::ReduceOp;
        use unpopped::{VariantFidelity, generate_variants};
...
{src}"
    ...
    fn precision_first_rowreduce_declines_f16_and_max_only() {
        use unpopped::generate_variants;
        use unpopped::ir::{ReduceOp, ReduceStage, reduced};
    ...
    fn contraction_emits_skinny_simt_kernel() {
        use unpopped::ir::{ContractionAxes, UnaryOp, reduced};
    ...
    fn emit_contraction_transposed_rhs_binding() {
        use unpopped::ir::{ContractionAxes, View, reduced};
    ...
    fn contraction_bias_emits_bias_param_and_column_load() {
        use unpopped::generate_variants;
        use unpopped::ir::{ContractionAxes, UnaryOp, input, reduced};
    ...
    fn splitk_gate_refuses_flipped_param_and_malformed_cells() {
        use unpopped::ir::ReduceOp;
        use unpopped::{Backend, generate_variants};
        use unpopped_vocab::AxisMask;
        let op = OpDef::reduction_axes(
            "sum",
            1,
    ...
    // the input offset.
    fn view_2d_key(n_operands: usize) -> unpopped_vocab::StructureKey {
    ...
    fn reduce_key(in_dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn reduction_mean_of_squares_f32() {
        use unpopped::ir::{ReduceOp, UnaryOp};
    ...
    fn reduction_count_s8_predicate_lowers_as_integer_not_float() {
        use unpopped::ir::{BinaryOp, ReduceOp, konst};
    ...
    fn reduction_post_norm2_applies_sqrt_after_the_sum() {
        use unpopped::ir::{ReduceOp, UnaryOp, reduced};
    ...
    fn reduce_key_hetero(out_dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn reduction_post_reading_input_is_rejected_at_the_plan_gate() {
        ...
        let _ = unpopped::build_plan(&op, &reduce_key(ElementKind::F32));
    }
    ...
    fn reduction_u8_out_non_cmp_post_is_rejected_at_the_plan_gate() {
        ...
        op.out_dtype = Some(ElementKind::U8);
        let _ = unpopped::build_plan(&op, &reduce_key_hetero(ElementKind::U8));
    }
    ...
    fn reduction_i64_out_non_sum_is_rejected_at_the_plan_gate() {
        ...
        op.out_dtype = Some(ElementKind::I64);
        let _ = unpopped::build_plan(&op, &reduce_key_hetero(ElementKind::I64));
    }
    ...
    fn rowreduce_prod_stage_is_rejected_at_the_plan_gate() {
        ...
        let _ = unpopped::build_plan(&op, &rr_key(ElementKind::F32, OpCategory::Softmax));
    }

    fn rr_key(dt: ElementKind, cat: OpCategory) -> unpopped_vocab::StructureKey {
    ...
    fn mi_key(dt: ElementKind, n_col: usize) -> unpopped_vocab::StructureKey {
    ...
    fn softmax_bw_key(dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn layer_norm_bw_key(dt: ElementKind) -> unpopped_vocab::StructureKey {
    ...
    fn nextafter_f16_is_refused_in_a_row_reduce_epilogue() {
        use unpopped::ir::{BinaryOp, ReduceOp, ReduceStage, reduced};
        let op = OpDef::row_reduce(
            "na_rr",
            1,
            &[ElementKind::F16],
    ...
    fn nextafter_f16_is_refused_in_a_contraction_epilogue() {
        use unpopped::ir::{BinaryOp, ContractionAxes, konst, reduced};
    ...
    use unpopped::ir::{BinaryOp, konst};
    ...
    fn pred_key(dt: ElementKind) -> unpopped_vocab::StructureKey {
        let a = OperandDesc::new(1, &[1 << 20], &[1], dt, 256);
    ...
    //
    // These call unpopped::build_plan DIRECTLY, not generate(): the panic must
    ...
    fn coord_key_2d(dt: ElementKind, n_operands: usize) -> unpopped_vocab::StructureKey {
    ...
    fn coord_body_never_vectorizes_aligned_v4_cell_takes_strided() {
        ...
        let plan = unpopped::build_plan(&op, &key);
        assert!(
            matches!(plan.schedule, Schedule::Strided),
        ...
        assert!(matches!(
            unpopped::build_plan(&plain, &key).schedule,
    ...
    fn coord_under_reduction_access_is_refused_by_the_emitter_backstop() {
        use unpopped::backend::Backend;
        use unpopped::ir::ReduceOp;
        use unpopped::plan::{KernelPlan, ReduceAxisClass, Schedule};
        use unpopped_vocab::AxisMask;
        let key = reduce_key(ElementKind::F32);
        let body = (input(0) * coord(0)).0;
        let access = unpopped::ir::Access::Reduction {
            op: ReduceOp::Sum,
            axes: AxisMask::EMPTY,
            keepdim: false,
            post: unpopped::ir::ScalarExpr::Reduced(0),
            accum: unpopped::ir::ReductionAccum::Auto,
        };
        let plan = KernelPlan {
            op_name: "backstop",
            n_inputs: 1,
    ...
    /// the OUTPUT pointer.
    fn unary_contig_key(dt: ElementKind) -> unpopped_vocab::StructureKey {
        let a = OperandDesc::new(1, &[1 << 20], &[1], dt, 256);
        structure_key(OpCategory::UnaryElementwise, &[a, a], ArchSku::Sm89)
    }
    ...
    fn offset_composes_with_permute_view() {
        use unpopped::ir::{BaseOffset, View};
    ...
    fn offset_base_variant_carries_the_oob_launch_note() {
        ...
        let key = binary_key(ElementKind::F32);
        let free = unpopped::generate_variants(&add_op(&[ElementKind::F32]), &key, &Cuda);
        assert!(
    ...
    fn offset_arity_mismatch_is_rejected_at_the_plan_gate() {
        ...
        let _ = unpopped::build_plan(&op, &binary_key(ElementKind::F32));
    }

    #[test]
    #[should_panic(expected = "Elementwise-only")]
    fn offset_on_a_reduction_is_rejected_at_the_plan_gate() {
        use unpopped::ir::{BaseOffset, ReduceOp};
...
mod multi_output_tests {
    ...
    use crate::Cuda;
    use unpopped::generate;
    use unpopped::ir::{BinaryOp, OpDef, UnaryOp, input, konst};
    use unpopped_vocab::{
...
mod dropout_hetero_tests {
    ...
    use crate::Cuda;
    use unpopped::backend::Backend;
    use unpopped::ir::{Access, BaseOffset, BinaryOp, OpDef, WriteIndex, input, konst, param};
    use unpopped::plan::{KernelPlan, Schedule};
    ...
    fn hetero_plan_slots<'a>(
        key: &'a StructureKey,
        body0: &'a unpopped::ir::ScalarExpr,
        extra_bodies: &'a [unpopped::ir::ScalarExpr],
...
mod scan_tests {
    ...
    use crate::Cuda;
    use unpopped::ir::{Access, OpDef, ReduceOp};
    use unpopped::plan::Schedule;
    use unpopped::{build_plan, generate, generate_variants};
    use unpopped_vocab::{
        ArchSku, ElementKind, OpCategory, OperandDesc, StructureKey, structure_key,
    };
    ...
    fn blockscan_variant_offers_maxmin_and_integer_bit_identical() {
        ...
        for (op, dt) in [
            ...
            assert_eq!(
                bs.fidelity,
                unpopped::VariantFidelity::BitIdentical,
    ...
    fn dump_offset_sources() {
        use unpopped::ir::{BaseOffset, input};
        use unpopped_vocab::{ArchSku, OpCategory, OperandDesc, structure_key};
    ...
    ///   `cargo test -p baracuda-cuda-emit dump_scan_sources -- --ignored --nocapture`
...
{s}"
    ...
    ///   `cargo test -p baracuda-cuda-emit dump_window_sources -- --ignored --nocapture`
    #[test]
...
mod im2col_tests {
    ...
    fn im2col_base_golden_f32() {
        ...
        let p = unpopped::ir::OpDef::im2col_2d(
            "unfold",
            ElementKind::F32,
            (3, 3),
            (1, 1),
    ...
    fn im2col_flipped_output_refused_by_backstop() {
        ...
        let key = structure_key(OpCategory::UnaryElementwise, &[a, o], ArchSku::Sm89);
        let body = unpopped::ir::input(0).0;
        let plan = KernelPlan {
            op_name: "sneaky",
            n_inputs: 1,
            dtype: ElementKind::F32,
            ...
            extra_out_bodies: &[],
            extra_out_dtypes: &[],
            access: &unpopped::ir::Access::Im2Col {
                kernel: (3, 3),
                stride: (1, 1),
                pad: (1, 1),
                dilation: (1, 1),
    ...
    fn dump_im2col_sources() {
        ...
        for &dt in &[
            ...
            for &(gname, (kernel, stride, pad, dilation)) in geos {
                let p = unpopped::ir::OpDef::im2col_2d(gname, dt, kernel, stride, pad, dilation);
...
mod sort_tests {
    ...
    use crate::Cuda;
    use unpopped::ir::{OpDef, SortOrder};
    ...
    fn bitonic_asc_smem_pad_and_barriers() {
        ...
        assert_eq!(bt.fidelity, unpopped::VariantFidelity::BitIdentical);
    }
    ...
    fn topk_values_base_golden_f32() {
        ...
        use unpopped::ir::{Access, SortLimit, SortOut};
    ...
    fn topk_both_emitter_backstop_rejects_broadcast_out_idx() {
        ...
        use unpopped::backend::Backend;
        use unpopped::ir::{Access, BaseOffset, SortLimit, SortOut, WriteIndex};
        use unpopped::plan::{KernelPlan, Schedule};
        let a = OperandDesc::new(2, &[256, 128], &[128, 1], ElementKind::F32, 256);
    ...
    fn sort_both_is_an_honest_miss_no_contract() {
        ...
        let k = generate(&sc, &both_key(ElementKind::F32), &Cuda);
        assert!(
            unpopped::contract(&sc, &both_key(ElementKind::F32), &k, &Cuda).is_none(),
    ...
    fn topk_is_an_honest_miss_no_contract() {
        ...
        let k = generate(&sc, &topk_both_key(ElementKind::F32, 64), &Cuda);
        assert!(
            unpopped::contract(&sc, &topk_both_key(ElementKind::F32, 64), &k, &Cuda).is_none(),
    ...
    fn partial_select_variant_present_for_topk() {
        ...
        assert_eq!(ps.fidelity, unpopped::VariantFidelity::BitIdentical);
    ...
    /// the on-device validator `#include`s. Run with:
    ///   `cargo test -p baracuda-cuda-emit dump_sort_sources -- --ignored --nocapture`
...
mod select_tests {
    ...
    use crate::Cuda;
    use unpopped::generate;
    use unpopped::ir::{BinaryOp, OpDef, coord, input, konst, reduced};
    use unpopped_vocab::{
        ArchSku, ElementKind, OpCategory, OperandDesc, StructureKey, structure_key,
    };
