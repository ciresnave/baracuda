...
// baracuda vendor patch: upstream used `boost::math::ccmath::fabs` purely
// for a constexpr `fabs` (std::fabs isn't constexpr before C++23). We
// replace the single use with a constexpr ternary below and drop the
// Boost dependency entirely (baracuda's nvcc host toolchain has no Boost).
...
static constexpr uint16_t fp16_ieee_from_fp32_value(float f) {
  ...
  const float saturated_f = (f < 0.0f ? -f : f) * scale_to_inf;  // baracuda: constexpr fabs (was boost::math::ccmath::fabs)
