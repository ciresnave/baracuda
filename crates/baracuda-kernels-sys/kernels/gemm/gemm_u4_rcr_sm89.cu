...
//
// gemm_u4_rcr_sm89.cu — int4 (unsigned) GEMM with RCR layout, Identity
// epilogue, for sm_89.
//
// Thin instantiation of `baracuda_int4_rcr_sm89.cuh` with
// `<Int4Encoding::U4, float, Activation::None>`. Identical shape to the
// S4 trailblazer; differs only in the MMA operand encoding
// (`.u4.u4`) and the saturating-cast back to u4 (clamp `[0, 15]`).
...
int32_t baracuda_kernels_gemm_u4_rcr_sm89_run(
...
size_t baracuda_kernels_gemm_u4_rcr_sm89_workspace_size(
    int32_t /*m*/, int32_t /*n*/, int32_t /*k*/
) {
    return 0;
}

int32_t baracuda_kernels_gemm_u4_rcr_sm89_can_implement(
    int32_t m, int32_t n, int32_t k,
    const void * /*a*/, int64_t /*lda_bytes*/,
    const void * /*b*/, int64_t /*ldb_bytes*/,
    const void * /*c*/, int64_t /*ldc_bytes*/,
