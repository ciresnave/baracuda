// baracuda-kernels Phase 3 unary fanout: elementwise atanh for FP
// types.
//
// Implements `y = atanh(x)` over both contiguous tensors (fast path) and
...
struct AtanhFunctor<float> {
    __device__ __forceinline__ float operator()(float x) const { return atanhf(x); }
};

template <>
struct AtanhFunctor<double> {
    __device__ __forceinline__ double operator()(double x) const { return atanh(x); }
};

template <>
struct AtanhFunctor<__half> {
...
template <>
struct AtanhFunctor<__nv_bfloat16> {
...
BARACUDA_KERNELS_UNARY_POINTWISE_INSTANTIATE(
    unary_atanh_f32,
    float,
    baracuda::elementwise::AtanhFunctor<float>)
...
BARACUDA_KERNELS_UNARY_POINTWISE_INSTANTIATE(
    unary_atanh_f16,
    __half,
    baracuda::elementwise::AtanhFunctor<__half>)
...
BARACUDA_KERNELS_UNARY_POINTWISE_INSTANTIATE(
    unary_atanh_bf16,
    __nv_bfloat16,
    baracuda::elementwise::AtanhFunctor<__nv_bfloat16>)
...
BARACUDA_KERNELS_UNARY_POINTWISE_INSTANTIATE(
    unary_atanh_f64,
    double,
    baracuda::elementwise::AtanhFunctor<double>)
