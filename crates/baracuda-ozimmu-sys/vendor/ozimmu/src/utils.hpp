...
#include <stdexcept>
// baracuda patch (Phase 44): the upstream code includes <dlfcn.h> and
// <unistd.h> to support the LD_PRELOAD interception path. In the baracuda
// integration we statically link ozIMMU and call cuBLAS directly through
// baracuda-cublas-sys, so neither header is needed. Skipping them lets the
// vendored sources build on Windows (where <dlfcn.h> / <unistd.h> do not
// exist). The `OZIMMU_BARACUDA_DIRECT_LINK` define is set unconditionally by
// the baracuda build script; if you re-vendor for a non-baracuda consumer
// that wants the LD_PRELOAD shim, undefine it.
#ifndef OZIMMU_BARACUDA_DIRECT_LINK
#  include <dlfcn.h>
#  include <unistd.h>
#endif
...
// baracuda patch (Phase 44): the upstream `ozIMMU_get_function_pointer`
// uses `dlsym(RTLD_NEXT, ...)` to grab the real cuBLAS symbol from under
// ozIMMU's LD_PRELOAD shim. The baracuda integration statically links
// ozIMMU + cuBLAS into one library, so we expose a tiny adapter that
// resolves the handful of cuBLAS functions ozIMMU asks for to symbols
// supplied by the `csrc/ozimmu_shim.cu` translation unit. The shim
// implementation registers `cublasCreate_v2`, `cublasDestroy_v2`,
// `cublasGemmEx`, and `cublasGemmStridedBatchedEx` — the only names
// `ozIMMU_get_function_pointer` is called with in this vendored subset
// (cublas.cu and culip.cu, the LD_PRELOAD-only translation units, are
// dropped from the build).
#ifdef OZIMMU_BARACUDA_DIRECT_LINK
...
inline void *ozIMMU_get_function_pointer(const std::string function_name,
  ...
  (void)library_name;
  void *fp = ozimmu_baracuda_lookup_cublas_symbol(function_name.c_str());
  if (fp == nullptr) {
    ozIMMU_error("baracuda shim: unknown cuBLAS function requested: " +
                 function_name);
  }
  return fp;
}
