// cublas_helper.hpp — FP64 GEMM emulated via FP32 cuBLAS calls.
//
// The Ozaki path is the primary backend; this f32-emulated path is
// the reference fallback (`compute_mode_t::sgemm` and the
// strided-batched variant). Not normally selected by callers but
// kept for accuracy regression tests.
...
/// One-shot FP64-as-FP32 GEMM. Round-trips A, B, C through fp32 with
/// an in-place cuBLAS s/cgemm and converts back. Accuracy is "f32-ish".
template <class T>
cublasStatus_t
...
/// Strided-batched companion to `dgemm_f32`.
template <class T>
cublasStatus_t
