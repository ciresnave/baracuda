# alpha.75 publish script.
#
# The IR-expansion-ramp-COMPLETE release: baracuda-kernelgen gains Access::Window
# (sliding-window max/min/sum/avg pooling, ramp #7) and Access::RowSort (stable
# row sort/argsort, bitonic variant 1.59x the bespoke msort, ramp #8) — the final
# two ramp increments (0a-#8), each adversarially reviewed and on-device
# validated. Plus the cuvs-sys FFI docs and the rustfmt 2024 pin. See
# CHANGELOG.md (0.0.1-alpha.75) and the docs/fuel-* channel.
# baracuda-kernelgen + baracuda-kernels-bench are publish=false (not on the
# index), so the topo order is unchanged from alpha.71-.74 (68 crates).
#
...
$logFile = "target/publish_alpha75.log"
New-Item -ItemType Directory -Force -Path "target" | Out-Null
Set-Content -Path $logFile -Value "alpha.75 publish run started $(Get-Date)`n"
